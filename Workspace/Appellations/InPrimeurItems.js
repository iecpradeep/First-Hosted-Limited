...
function inPrimeurItems(params)
{
	...
	//filters.push(new nlobjSearchFilter('custitem_inprimeur', null, 'is', 'T'));
	
	//filters.push(new nlobjSearchFilter("31","custrecord_rotation_enprimeur", 'is', 'T'));
	
	var itemsList = nlapiSearchRecord('item', 61, filters);// Make the search
	if(itemsList != null && itemsList != undefined)
	{			
		mycount=itemsList.length;
		if(myshort==1 && itemsList.length>25) mycount=25;
	}	
	...
	if(itemsList != null && itemsList != undefined)
	{
		...
		for(var i = 0; i < mycount; i++) {
			...
			if(itemsList[i].getValue(columns[26]))
				//img = itemsList[i].getValue(columns[26]);
			/*try
			{*/
...
//				}
			...
			catch(e)
			{
				...
				content += '<td>'+ (itemsList[i].getValue(columns[15])==''?'': ' ' + itemsList[i].getText(columns[15])) +'</td><td>'+ itemsList[i].getText(columns[5]) +'</td><td style="display:none;"> '+itemsList[i].getText(columns[2]) +'</td><td>'+itemsList[i].getText(columns[21]) + (itemsList[i].getValue(columns[14])==''?'': ' ' + itemsList[i].getText(columns[14])) +'</td><td>£'+itemsList[i].getValue(columns[4])+'</td><td>'+itemsList[i].getText(columns[7])+'</td><td>'+(itemsList[i].getValue(columns[16])==''?'':itemsList[i].getText(columns[16]))+'</td><td>'+avl+'</td><td>'+itemsList[i].getValue(columns[8])+'</td><td>'+(itemsList[i].getValue(columns[22])==""?' ':itemsList[i].getText(columns[22]))+'</td><td>'+(itemsList[i].getValue(columns[17])==''?'':itemsList[i].getText(columns[17]))+'</td><td><a href="'+itemsList[i].getValue(columns[18])+'?isinpremiur=T" class="call-to-action_button" >BUY NOW</a></td>';
			}*/
	...
	if(itemsList != null && itemsList != undefined)
	{
		content += "<tr style='display:none'><td id='pageNumber'>1</td><td id='itemsQuantity'>" + mycount + "</tr>";
	}
	else
	{
