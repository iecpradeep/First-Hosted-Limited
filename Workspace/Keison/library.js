 ...
 * 1.1.0 - 24/04/2013 - Added new block "updatedDespositedamount" -SA
 *
 * Author:	FHL
 ...
 */
function createJournal(totalValue, toAcc, fromAcc, dept, location, subsidiary, jClass, invDate,  desc, entity)
{
	...
	try
	{
		...
		currency = 1;	//british Pound
		
		nlapiLogExecution('debug', 'createJournal currency testingNum ' + testingNum, currency);
		
		journalRecord.setFieldValue('currency',currency);		// UK [TODO] not hardcoded please!!
 ...
 * 
 * 1.1.0 - adding getCashSaleLineFields(lineNumber),getCashSaleFields(), getCompanyInformation() functions - AS
 *************************************************************/
...
/**************************************************************
 * getCashSaleInformation - getting the cash sale body fields
 * 
 * 1.1.0 - adding getCashSaleLineFields(lineNumber),getCashSaleFields(), getCompanyInformation() functions - AS
 *************************************************************/
function getCashSaleFields()
{
	try
	{
			// get required header fields from fulfillment record and escape into xml
			cashSalePartner = nlapiEscapeXML(cashSaleRecord.getFieldText('partner'));
			cashSaleNumber = nlapiEscapeXML(cashSaleRecord.getFieldValue('tranid'));
			cashSaleBillTo = nlapiEscapeXML(cashSaleRecord.getFieldValue('billaddress'));
			
			//if the variable value is null, set it as '', to avoid it showing as null in the pdf
			if(cashSaleBillTo == null)
			{
				cashSaleBillTo = '';				
			}
			
			cashSaleDate = nlapiEscapeXML(cashSaleRecord.getFieldValue('trandate'));
			paymentMethod = nlapiEscapeXML(cashSaleRecord.getFieldText('paymentmethod'));
			...
			//vatregnum
			taxid = nlapiEscapeXML(cashSaleRecord.getFieldValue('vatregnum'));
			...
			chequeNo = nlapiEscapeXML(cashSaleRecord.getFieldValue('otherrefnum'));
			...
			project = nlapiEscapeXML(cashSaleRecord.getFieldValue('job'));
			...
			startDate = nlapiEscapeXML(cashSaleRecord.getFieldValue('startdate'));
			...
			endDate = nlapiEscapeXML(cashSaleRecord.getFieldValue('enddate'));
			...
			numOfItems = cashSaleRecord.getLineItemCount('item');
			distributorRate = cashSaleRecord.getFieldValue('discountrate');
	...
	{
		errorHandler('getCashSaleFields', e);
	}
}
...
/******************
 * updated deposited amount (deposit amount- refund amount)
 * 1.1.0 - 24/04/2013 - Added new block "updatedDespositedamount" -SA
 * 
 ******************/
function updatedDepositedAmount(salesOrderID)
{

	var salesOrderTotal = 0.00; 
	var CDSearchFilters = new Array();
	var CDSearchColumns = new Array();

	var CDSearchResults = null;

	var totalDepositted = 0.00;
	var percentDepositted = 0;
	var totalRefunded = 0.00;

	var depositRecord = null;
	var depositRecordId = 0;

	var netDepositAmount =0.00;

	try
	{

		salesOrderTotal = nlapiLookupField('salesorder',salesOrderID, 'fxamount');

		// search - all the customer deposits, which is applied to this particular sales order, and get the deposit amount and the total refund amount.
		CDSearchFilters[0] = new nlobjSearchFilter('salesorder', null, 'anyof', salesOrderID );

		CDSearchColumns[0] = new nlobjSearchColumn('fxamount');   //1.0.1 
		CDSearchColumns[1] = new nlobjSearchColumn('internalid'); 

		// perform search
		CDSearchResults = nlapiSearchRecord('customerdeposit', null, CDSearchFilters, CDSearchColumns);

		if (CDSearchResults)
		{
			for (var j=0; j<CDSearchResults.length; j++)
			{			

				totalDepositted += parseFloat(CDSearchResults[j].getValue(CDSearchColumns[0]));
				//totalRefunded  = (CDSearchResults[j].getValue(CDSearchColumns[1]));

				depositRecordId= CDSearchResults[j].getValue(CDSearchColumns[1]);

				depositRecord = nlapiLoadRecord('customerdeposit', depositRecordId);
				// get the refund amount
				totalRefunded+= parseFloat(depositRecord.getFieldValue('custbody_refundtotal'));	
			}

		}
		// calculate the nedeposit amount, which take into account of refunds.
		netDepositAmount = totalDepositted -totalRefunded;
		percentDepositted = ((netDepositAmount / salesOrderTotal)*100);

		nlapiSubmitField('salesorder', salesOrderID, 'custbody_kil_tran_deposittedamount', netDepositAmount.toFixed(2), false); 
		nlapiSubmitField('salesorder', salesOrderID, 'custbody_kil_tran_deposittedpercent', percentDepositted.toFixed(2), false); 

	}
	catch(e)
	{
		errorHandler('updatedDepositedAmount', e);
	}     	      
