 ...
 * @param {Object} column
 */
function H1ForecastTeamOne(portlet, column)
{   
...
//	Employee search
	...
	for(var emp = 0; emp < numOfEmployees; emp++)
	{
		...
		try
		{
			...
			content += "<td align='right' BGCOLOR='" + row_change(emp) + "' title='" + thisyear() + " H1 Target for " + SalesRepName + " is $" 
			...
			content += "<td align='right' BGCOLOR='" + row_change(emp) + "' title='% Target Achieved for H1 " + thisyear() + " for " + SalesRepName + " is " + EmpFYTotal + "%'><font size='2.4'>" + EmpFYTotal + "%</font></td>";
...
//		Altered Verbal Weighting
		...
		content+= "<td align='right' BGCOLOR='"+row_change(numOfEmployees) +"' title='" + thisyear() + " H1 Target is $" + SumTargetTotal.formatMoney(0, '.', ',')  
