 ...
 * Purpose:			Year to Date Sales by Sales Rep, All Teams
 ...
 * @param {Object} column
 */
function H1ByTypeAllTeams(portlet, column)
{   
...
//	/These are to be reset every iteration of the employee
	...
	portlet.setTitle('H1 Result by Type of Sale - ' + TeamName);// Teams Included: ' + TheTeam.length + " - Types Len: " + Types.length);
	...
	for(var TheType = 0; TheType < Types.length; TheType++)
	{
		...
		try {
			...
			content+= "<td align='right' BGCOLOR='"+row_change(TheType) +"' title='H1 Target  for " + TypeName + " is $" + TypeTargetTotal + "'><font size='2.4'>$" 
	...
	content += "<td align='right' title='Total deals required to reach the " + thisyear() + " target is " 
			+ SumCADTotal.formatMoney(0,".",",") + "'><font size='2.4'><u><b>";
