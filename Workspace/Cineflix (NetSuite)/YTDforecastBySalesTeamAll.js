 ...
 * Purpose:			Year to date forecast by Sales Rep, all teams
 * Script Type:		Javascript
 * Version:			1.7.000
 * Date:			November 2011 - 7th June 2012
 * Author:		    Pete Lewis, First Hosted Limited.
 * History:
 * 1.7: Changed "Team 1" to "Paul's Team" and fix IsNegative issue
 * 1.8 19/07/2012 - Altered the Verbal Weighting
 * 					line 549. - A.D.M
 *******************************************************/
...
var content = "";
var TheTeam = new Array(); // team number internal ID
var TeamNumber = 5; // internal ID of the team number ** see customlist_team **
...
{	
	...
	if(allowPortCust == 'T')
	{
		...
		if (InDebug) 
		{
			...
			{
				content += "<p> NO PORTLET SETTINGS FOUND </p>";
			}
		}	
		...
		else		
		{
			if (InDebug) 
			{
				content += "<p>Portlett settings found.. test result: " + portSearchResults[0].getValue(portSearchColumns[3]) + "</p>";
			}
		}
	}
	else
 ...
 * @param {Object} column
 */
function ytdForecastallteam(portlet, column)
{   
...
//	Employee search
	...
	{
		portlet.setHtml( "<p>It seems there are no Sales Reps in " +  TeamName + "</p>" );
		return true;
	}
	...
	portlet.setTitle('Year to Date Forecast by Sales Person - '+ TeamName);
	...
	content+= "<td align='right' title='Likely Outcome for " + thisyear() + "'><font size='2.4'><u><b> Likely Outcome for "+thisyear()+"</b></u></font></td>";
	content+= "<td align='right' title='" + thisyear() + " Full Year Target'><font size='2.4'><u><b>"+thisyear()+" Full Year Target</b></u></font></td>";
	content+= "<td align='right' title='% Target Achieved for " + thisyear() + "'><font size='2.4'><u><b>% "+thisyear()+" Target Achieved</b></u></font></td>";
	...
	for(var emp = 0; emp < numOfEmployees; emp++)
	{
		...
		try {
			...
			content+= "<td align='right' BGCOLOR='" + row_change(emp) + "' title='Verbally Agreed Total for " + SalesRepName + " is $" + parseInt(EmpVerbalTotal * Verbal)  
					+ "'><font size='2.4'>$" + parseInt(EmpVerbalTotal * Verbal).formatMoney(0, '.', ',')   + "</font></td>";
			
			content+= "<td align='right' BGCOLOR='" + row_change(emp) + "' title='Deal Memo Total for " + SalesRepName + " is $" + parseInt(EmpDealMemoTotal * DealMemo) 
					+ "'><font size='2.4'>$" + parseInt(EmpDealMemoTotal * DealMemo).formatMoney(0, '.', ',')  + "</font></td>";
			
			content+= "<td align='right' BGCOLOR='" + row_change(emp) + "' title='In Negotiation Total for " + SalesRepName + " is $" + parseInt(EmpInNegTotal * InNegotiation) 
					+ "'><font size='2.4'>$" + parseInt(EmpInNegTotal * InNegotiation).formatMoney(0, '.', ',') + "</font></td>";
			
			content+= "<td align='right' BGCOLOR='" + row_change(emp) + "' title='Actual Total for " + SalesRepName + " is $" + EmpSignedTotal  + "'><font size='2.4'>$" 
					+ EmpSignedTotal.formatMoney(0, '.', ',')  + "</font></td>";
			...
			content+= "<td align='right' BGCOLOR='"+row_change(emp) +"' title='Likely Outcome for " + thisyear() + " for " + SalesRepName + " is $" + EmpLikelyTotal.formatMoney(0, '.', ',') + "'><font size='2.4'>$" + EmpLikelyTotal.formatMoney(0, '.', ',') + "</font></td>"; 
			content+= "<td align='right' BGCOLOR='"+row_change(emp) +"' title='" + thisyear() + " Full Year Target for " + SalesRepName + " is $" + EmpTargetTotal.formatMoney(0, '.', ',')  + "'><font size='2.4'>$" + EmpTargetTotal.formatMoney(0, '.', ',')  + "</font></td>";
			...
			content+= "<td align='right' BGCOLOR='"+row_change(emp) +"' title='% Target Achieved for " + thisyear() + " for " + SalesRepName + " is " + EmpFYTotal + "%'><font size='2.4'>" + EmpFYTotal + "%</font></td>";
			EmpCADTotal += parseInt(EmpTargetTotal - EmpLikelyTotal);
			content+= "<td align='right' BGCOLOR='"+row_change(emp) +"' title='Deals Required to reach target for " + SalesRepName + " is " + EmpCADTotal + "'><font size='2.4'>" + isNegative(EmpCADTotal) + "</font></td></tr>";
		} 
		catch (e) 
		{
	...
	try
	{
		...
		if( Initiated != 0)
		{
			content+= "<td align='right' BGCOLOR='" +row_change(numOfEmployees) + "' title='Initiated Total is $" 
					+ parseInt(SumInitiatedTotal * Initiated).formatMoney(0, '.', ',') + "'><font size='2.4'><u><b>$" 
					+ parseInt(SumInitiatedTotal * Initiated).formatMoney(0, '.', ',') + "</b></u></font></td>" ;
		}
		if( Pending != 0)
		{
...
//		Altered Verbal Weighting - AM
		...
		content+= "<td align='right' BGCOLOR='"+row_change(numOfEmployees) +"' title='Likely Outcome for " + thisyear() + " is $" + SumLikelyTotal.formatMoney(0, '.', ',') 
				+ "'><font size='2.4'><u><b>$" + SumLikelyTotal.formatMoney(0, '.', ',') + "</b></u></font></td>"; 
		
		content+= "<td align='right' BGCOLOR='"+row_change(numOfEmployees) +"' title='" + thisyear() + " Full Year Target is $" + SumTargetTotal.formatMoney(0, '.', ',')  
		...
		content+= "<td align='right' BGCOLOR='"+row_change(emp) +"' title='% Target Achieved for " + thisyear() + " is " + SumFYTotal + "%'><font size='2.4'><u><b>" + SumFYTotal + "%</b></u></font></td>";
		SumCADTotal += parseInt(SumTargetTotal - SumLikelyTotal);
		content+= "<td align='right' BGCOLOR='"+row_change(emp) +"' title='Deals Required to reach target is $" + SumCADTotal.formatMoney(0, '.', ',') + "'><font size='2.4'><u><b> " + isNegative(SumCADTotal) + "</b></u></font></td></tr>";
	}
	catch(e)
	{
...
function getNSFiscalEndDate()
{
	...
	if (Today.getMonth() >= 9)
	{
		NSFiscalEnd = "30/9/" + String(parseInt(EndDate + 1));
	}	 
	else
	{
		NSFiscalEnd = "30/9/" + String(parseInt(EndDate));
	}	
	return NSFiscalEnd;
}
