 ...
 * Purpose:			H1 Forecast by Type of Sale, Team two
 * Script Type:		Javascript
 * Version:			1.7.5
 * 					1.7.6 29/06/2012 Changed getNSFiscalEndDate 
 * 					to only be within 6 months. - A.D.M
 *  				1.7.7 19/07/2012 - Altered the Verbal Weighting
 * 					line 511 and commented out SumLikelyTotal
 * 					line 378. - A.D.M	
 * 
 * Date:			December 2011 - 7th June 2012
...
var TeamNumber = 1; // internal ID of the team number ** see customlist_team **

/********************************************************
 *  Team numbers in custom record - update as approriate
 * 
 * Pauls team == 1 
 * Glens team == 2
 * Testing team == 3
...
var TheTeam = 1; //change this if you wish...
...
function getPortletSettings(userId)
{
	...
	if(allowPortCust == 'T')
	{
		...
		if (InDebug) 
		{
			...
			{
				content += "<p>ERROR NO PORTLET SETTING RESULTS FOUND</p>";
			}
 ...
 * @param {Object} column
 */
function YTDforecastTypeOfSaleTeam1(portlet, column)
{   
...
//	var EmpFYTotal = 0;
	...
	portlet.setTitle('Year to Date Forecast by Type of Sale - ' + TeamName);
	...
	content+= "<td align='right' title='Likely Outcome for " + thisyear() + "'><font size='2.4'>&nbsp<u><b> Likely Outcome for "+thisyear()+"</b></u></font></td>";
	content+= "<td align='right' title='" + thisyear() + " Full Year Target'><font size='2.4'>&nbsp<u><b>"+thisyear()+" Full Year Target</b></u></font></td>";
	content+= "<td align='right' title='% Target Achieved for " + thisyear() + "'><font size='2.4'>&nbsp<u><b>% "+thisyear()+" Target Achieved</b></u></font></td>";
	...
	for(var TheType = 0; TheType < Types.length; TheType++)
	{
		...
		//content+= "<p>TypeID: " + TheType + ", TypeName: " + TypeName + "</p>";
		...
		estfilter[1] = new nlobjSearchFilter('custentity_team', 'salesrep','anyOf', TheTeam);
		...
		var searchresults = nlapiSearchRecord('estimate', null, estfilter, [amount, type_of_salecolumn, enddate, entitystatus]);
		...
		else
		{
			for(var thisEstimate = 0; thisEstimate < searchresults.length; thisEstimate++)
			{
				...
				{
					TypeInNegTotal  += parseInt(searchresults[thisEstimate].getValue('amount'));
...
//				Commented out - AM
				...
				if(InDebug)
				{
					content+="<p>SumLikelyTotal </p>" + SumLikelyTotal;
				}
		...
		{	
			...
			dealfilter[1] = new nlobjSearchFilter('custentity_team', 'salesrep','anyOf', TheTeam);
...
//					}
		...
		try 
		{
			...
			oppfilter[4] = new nlobjSearchFilter('custentity_team', 'salesrep','anyOf', TheTeam);
...
//			Altered Verbal Weighting - AM
			content+= "<td align='right' BGCOLOR='" + row_change(TheType) + "' title='Verbally Agreed Total for " + TypeName + " is $" + parseInt(TypeVerbalTotal * Verbal)  
					+ "'><font size='2.4'>&nbsp$" + parseInt(TypeVerbalTotal * Verbal).formatMoney(0, '.', ',')   + "</font></td>";
			...
			content+= "<td align='right' BGCOLOR='" + row_change(TheType) + "' title='In Negotiation Total for " + TypeName + " is $" + parseInt(TypeInNegTotal * InNegotiation) 
					+ "'><font size='2.4'>&nbsp$" + parseInt(TypeInNegTotal * InNegotiation).formatMoney(0, '.', ',')   + "</font></td>";
			content+= "<td align='right' BGCOLOR='" + row_change(TheType) + "' title='Actual Total for " + TypeName + " is $" + TypeSignedTotal  + "'><font size='2.4'>&nbsp$" 
					+ TypeSignedTotal.formatMoney(0, '.', ',')  + "</font></td>";
			...
			if(InDebug)
			{
				content+="<p>1 - TypeLikelyTotal Again</p>" + TypeLikelyTotal;
				content+="<p>1 - SumLikelyTotal Again</p>" + SumLikelyTotal;
			}

			content+= "<td align='right' BGCOLOR='"+row_change(TheType) +"' title='Likely Outcome for " + thisyear() + " for " + TypeName + " is $" 
					+ TypeLikelyTotal.formatMoney(0, '.', ',') + "'><font size='2.4'>$" + TypeLikelyTotal.formatMoney(0, '.', ',') + "</font></td>"; 
			content+= "<td align='right' BGCOLOR='"+row_change(TheType) +"' title='" + thisyear() + " Full Year Target for " + TypeName + " is $" 
			...
			content+= "<td align='right' BGCOLOR='"+row_change(TheType) +"' title='% Target Achieved for " + thisyear() + " for " + TypeName + " is " 
					+ TypeFYTotal + "%'><font size='2.4'>" + TypeFYTotal + "%</font></td>";


			TypeCADTotal += parseInt(TypeTargetTotal - TypeLikelyTotal);

			content+= "<td align='right' BGCOLOR='"+row_change(TheType) +"' title='Deals Required to reach target for " + TypeName + " is " 
					+ TypeCADTotal + "'><font size='2.4'>" + isNegative(TypeCADTotal) + "</font></td></tr>";
		} 
		catch (e) 
		{
	...
	try
	{
		...
		content+= "<td align='right' BGCOLOR='"+row_change(Types.length) +"' title='Verbally Agreed Total is $" + parseInt(SumVerbalTotal * Verbal).formatMoney(0, '.', ',')  
...
content+= "<td align='right' BGCOLOR='"+row_change(Types.length) +"' title='In Negotiation Total is $" + parseInt(SumInNegTotal*InNegotiation).formatMoney(0, '.', ',')   
		...
		content+= "<td align='right' BGCOLOR='"+row_change(Types.length) +"' title='Likely Outcome for " + thisyear() + " is $" + SumLikelyTotal.formatMoney(0, '.', ',') + "'><font size='2.4'><u><b>$" + SumLikelyTotal.formatMoney(0, '.', ',') + "</b></u></font></td>"; 
		content+= "<td align='right' BGCOLOR='"+row_change(Types.length) +"' title='" + thisyear() + " Full Year Target is $" + SumTargetTotal.formatMoney(0, '.', ',')  + "'><font size='2.4'><u><b>$" + SumTargetTotal.formatMoney(0, '.', ',')  + "</b></u></font></td>";
		...
		content+= "<td align='right' BGCOLOR='"+row_change(Types.length) +"' title='% Target Achieved for " + thisyear() + " is " + SumFYTotal + "%'><font size='2.4'><u><b>" + SumFYTotal + "%</b></u></font></td>";


		SumCADTotal += parseInt(SumTargetTotal - SumLikelyTotal);


		content+= "<td align='right' BGCOLOR='"+row_change(Types.length) +"' title='Deals Required to reach target is $" + SumCADTotal.formatMoney(0, '.', ',') + "'><font size='2.4'><u><b> " + isNegative(SumCADTotal) + "</b></u></font></td></tr>";
...
{	
	...
	if (Today.getMonth() >= 9)
	{
		NSFiscalEnd = "30/9/" + String(parseInt(Today.getFullYear() + 1));
	}	 
	else
	{
		NSFiscalEnd = "30/9/" + String(parseInt(Today.getFullYear()));
	}
	return NSFiscalEnd;
}
