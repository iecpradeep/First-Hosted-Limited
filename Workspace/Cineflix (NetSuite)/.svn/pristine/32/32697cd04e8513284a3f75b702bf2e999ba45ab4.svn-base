 ...
 * Purpose:			H1 Sales by Sales Rep, Team one
 * Script Type:		Javascript
 * Version:			1.8
 * 					1.8.1 29/06/2012 Changed getNSFiscalEndDate 
 * 					to only be within 6 months. - A.D.M
 * 					1.8.2 29/06/2012 Hard code the Team number 
 * 					filters dealfilter[5] and estfilter[6] and oppfilter[4]
 * 					[HACK]. - A.D.M	
...
/*****************************************
 * H1 Sales by Type of Sale - Team One
 * 
 * @param {Object} portlet
 * @param {Object} column
 * 
 *****************************************/
function H1ByTypeTeamOne(portlet, column)
{   
...
//	/These are to be reset every iteration of the employee
	...
	"<td align='right' title='Deals Required in CAD$ to reach target'><font size='2.4'><u><b>CAD$ of Deals Req.</b></u></font></td></tr>";
			...
			if(InDebug)
			{
				content+="<p>(Debug) Get NS Details (NSFSD): " + getNSFiscalStartDate() + "<br>(NSFED): " + getNSFiscalEndDate() + "</p>";
			}
	...
	for(var TheType = 0; TheType < Types.length; TheType++)
	{
		...
		//2.0.0 PAL - Changed 1 to TheTeam
		...
		nlapiLogExecution('ERROR', 'Search Results', searchresults);
		...
		{	
			...
			var teamNum = new nlobjSearchColumn('custentity_team');
		...
		try 
		{
			...
			//2.0.0 PAL - changed trandate to expectedclosedate
...
//						content += "<p><u>Sum Target " + SumTargetTotal + " </u></p>";
		...
		try {
			...
			content+= "<td align='right' BGCOLOR='"+row_change(TheType) +"' title='H1 Target  for " + TypeName + " is $" + TypeTargetTotal + "'><font size='2.4'>$" + parseInt(TypeTargetTotal).formatMoney(0,".",",") +  "</font></td>";
