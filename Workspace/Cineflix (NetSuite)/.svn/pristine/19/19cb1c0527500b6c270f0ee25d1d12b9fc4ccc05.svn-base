 ...
 * Purpose:			H1 Sales by Sales Rep, Team Two
 * Script Type:		Javascript
 * Version:			1.8
 * 					1.8.1 30/06/2012 Changed getNSFiscalEndDate 
 * 					to only be within 6 months. - A.D.M	
 * 					1.8.2 29/06/2012 Hard code the Team number 
 * 					filters dealfilter[5] and estfilter[6] 
 * 					[HACK]. - A.D.M	
 ...
 * @param {Object} column
 */
function H1ByTypeTeamTwo(portlet, column)
{   
...
//	/These are to be reset every iteration of the employee
	...
	for(var TheType = 0; TheType < Types.length; TheType++)
	{
		...
		estfilter[6] = new nlobjSearchFilter('custentity_team', 'salesrep', 'is', TheTeam ); //2.0.0 PAL
		...
		{	
			...
			dealfilter[5] = new nlobjSearchFilter('custentity_team', 'salesrep', 'is', TheTeam ); //2.0.0 PAL
		...
		try 
		{
			...
			oppfilter[4] = new nlobjSearchFilter('custentity_team', 'salesrep', 'is', TheTeam);	//2.0.0 PAL
