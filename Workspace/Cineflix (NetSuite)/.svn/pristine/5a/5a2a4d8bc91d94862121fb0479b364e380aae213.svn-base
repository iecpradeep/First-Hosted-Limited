 ...
 * Purpose:			Year to Date Forecast by Type of Sale, All Teams
 * Script Type:		Javascript
 * Version:			1.7.000
 * 					1.7.001 29/06/2012 Changed getNSFiscalEndDate 
 * 					to only be within 6 months. - A.D.M
 * 					1.7.002 19/07/2012 - Altered the Verbal Weighting
 * 					line 558 and commented out SumLikelyTotal
 * 					line 355. - A.D.M	
 * 
 * Date:			December 2011 - 12th March 2012
...
var TeamNumber = 5; // internal ID of the team number ** Team ALL **
 ...
 * @param {Object} column
 */
function YTDforecastTypeOfSaleAllTeams(portlet, column)
{   
...
//				Commented out - AM
		...
		try
		{
			...
			content+= "<td align='right' BGCOLOR='"+row_change(TheType) +"' title='% Target Achieved for " + thisyear() + " for " + TypeName + " is " + TypeFYTotal 
					+ "%'><font size='2.4'>" + TypeFYTotal + "%</font></td>";


			TypeCADTotal += parseInt(TypeTargetTotal - TypeLikelyTotal);

			content+= "<td align='right' BGCOLOR='"+row_change(TheType) +"' title='Deals Required to reach target for " + TypeName + " is " + TypeCADTotal 
...
//		Altered Verbal Weighting - AM
		...
		content+= "<td align='right' BGCOLOR='"+row_change(Types.length) +"' title='Likely Outcome for " + thisyear() + " is $" + SumLikelyTotal.formatMoney(0, '.', ',') 
				+ "'><font size='2.4'><u><b>$" + SumLikelyTotal.formatMoney(0, '.', ',') + "</b></u></font></td>"; 
		
		content+= "<td align='right' BGCOLOR='"+row_change(Types.length) +"' title='" + thisyear() + " Full Year Target is $" + SumTargetTotal.formatMoney(0, '.', ',')  
		...
		content+= "<td align='right' BGCOLOR='"+row_change(Types.length) +"' title='% Target Achieved for " + thisyear() + " is " + SumFYTotal 
		...
		content+= "<td align='right' BGCOLOR='"+row_change(Types.length) +"' title='Deals Required to reach target is $" + SumCADTotal.formatMoney(0, '.', ',') 
...
{	
	...
	if(InDebug)
		{
		content+="<p>NSFiscalEnd: " + NSFiscalEnd + "</p>";
		}
	return NSFiscalEnd;
}
