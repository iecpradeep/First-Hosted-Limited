 ...
 * @param {Object} column
 */
function ytdForecastTeam2(portlet, column)
{   
	...
	for(var emp = 0; emp < numOfEmployees; emp++)
	{
		...
		try
		{
			...
			content += "<td align='right' BGCOLOR='" + row_change(emp) + "' title='In Negotiation Total for " + SalesRepName + " is $" 
					+ parseInt(EmpInNegTotal * InNegotiation) + "'><font size='2.4'>$" + parseInt(EmpInNegTotal * InNegotiation).formatMoney(0, '.', ',') 
					+ "</font></td>";
			...
			content += "<td align='right' BGCOLOR='" + row_change(emp) + "' title='% Target Achieved for " + thisyear() + " for " + SalesRepName + " is " 
					+ EmpFYTotal + "%'><font size='2.4'>" + EmpFYTotal + "%</font></td>";
