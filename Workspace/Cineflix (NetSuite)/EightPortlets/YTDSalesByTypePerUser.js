/*******************************************************
 * Name: YTDSalesByTypePerUser.js - Cineflix Custom Portlet script
 * Script Type:	Client
 *
 * Version:	1.0.1 – 11/07/2012 – 1st release - AM
 *
 * Author:	FHL
 * Purpose:	YTD Sales by Type Per User - gathers information 
 * 			of the type of sales from salesorder, opportunity, 
 * 			estimates records regarding the user if they are 
 * 			a Sales Rep and within the fiscal year.
 *          
 *******************************************************/
 ...
 * @param {Object} column
 */
function YTDByTypeUser(portlet, column)
{  
	var noResults = true;
	var Types = new Array();
	var TheType = 0;
...
//	/These are to be reset every iteration of the employee
	...
	portlet.setTitle('Year to Date Result by Type of Sale');
	...
	for(TheType = 0; TheType < Types.length; TheType++)
	{
		...
		try
		{
			...
			estfilter[6] = new nlobjSearchFilter('salesrep', null, 'is', userId); //[HACK]
		...
		{	
			...
			dealfilter[5] = new nlobjSearchFilter('salesrep', null, 'is', userId ); //[HACK]
		...
		try 
		{
			...
			oppfilter[4] = new nlobjSearchFilter('salesrep', null, 'is', userId ); //[HACK]
...
//					}
		...
		try {
			...
			content+= "<td align='left' BGCOLOR='"+row_change(TheType) +"' title='Type of Sale: " + TypeName + ", Team: " + TheTeam +"'><font size='2.4'>" 
					+ TypeName +  "</font></td>";
			
			content+= "<td align='right' BGCOLOR='"+row_change(TheType) +"' title='Q1 Actual for " + TypeName + " is $" + TypeQ1Total + "'><font size='2.4'>$" 
					+ parseInt(TypeQ1Total).formatMoney(0,".",",")   +  "</font></td>" ;
			
			content+= "<td align='right' BGCOLOR='"+row_change(TheType) +"' title='Q2 Actual  for " + TypeName + " is $" + TypeQ2Total + "'><font size='2.4'>$" 
					+ parseInt(TypeQ2Total).formatMoney(0,".",",")  +  "</font></td>";
			
			content+= "<td align='right' BGCOLOR='"+row_change(TheType) +"' title='Q3 Actual  for " + TypeName + " is $" + TypeQ3Total + "'><font size='2.4'>$" 
					+ parseInt(TypeQ3Total).formatMoney(0,".",",") +  "</font></td>";
			
			content+= "<td align='right' BGCOLOR='"+row_change(TheType) +"' title='Q4 Actual  for " + TypeName + " is $" + TypeQ4Total + "'><font size='2.4'>$" 
					+ parseInt(TypeQ4Total).formatMoney(0,".",",")  +  "</font></td>";
			
			content+= "<td align='right' BGCOLOR='"+row_change(TheType) +"' title='Total YTD  for " + TypeName + " is $" + TypeFYTotal + "'><font size='2.4'>$" 
					+ parseInt(TypeFYTotal).formatMoney(0,".",",") +  "</font></td>";
			
			content+= "<td align='right' BGCOLOR='"+row_change(TheType) +"' title='Full Target  for " + TypeName + " is $" + TypeTargetTotal + "'><font size='2.4'>$" 
					+ parseInt(TypeTargetTotal).formatMoney(0,".",",") +  "</font></td>";
	...
	content += "%</b></u></font></td>";
	content += "<td align='right' title='Total deals required to reach the " + thisyear() + " target is " + SumCADTotal.formatMoney(0,".",",") 
			+ "'><font size='2.4'><u><b>";
