/*******************************************************
 * Name: 		YTDForecastBySalesPerUser.js - Cineflix Custom Portlet script
 * Script Type:	Client
 *
 ...
 *
 * Author:		FHL
 * Purpose:		YTD Forecast by Sales Per User - To display the forecasts
 * 				for the fiscal year period for user.
 *          
 *******************************************************/
 ...
 * @param {Object} column
 */
function ForecastBySalesUser(portlet, column)
{   
	// get the current user id
	var userId = nlapiGetUser(); 
	portSuccess = getPortletSettings(userId);
...
//	Employee search
	...
	for(var emp = 0; emp < numOfEmployees; emp++)
	{
		...
		try
		{
			...
			content += "<td align='right' BGCOLOR='" + row_change(emp) + "' title='Likely Outcome for " + thisyear() + " for " + SalesRepName + " is $" 
					+ EmpLikelyTotal.formatMoney(0, '.', ',') + "'><font size='2.4'>$" + EmpLikelyTotal.formatMoney(0, '.', ',') + "</font></td>";
			content += "<td align='right' BGCOLOR='" + row_change(emp) + "' title='" + thisyear() + " Full Year Target for " + SalesRepName + " is $" 
			...
			content += "<td align='right' BGCOLOR='" + row_change(emp) + "' title='% Target Achieved for " + thisyear() + " for " + SalesRepName + " is " + EmpFYTotal 
					+ "%'><font size='2.4'>" + EmpFYTotal + "%</font></td>";
			EmpCADTotal += parseInt(EmpTargetTotal - EmpLikelyTotal);
			content += "<td align='right' BGCOLOR='" + row_change(emp) + "' title='Deals Required to reach target for " + SalesRepName + " is " + EmpCADTotal 
					+ "'><font size='2.4'>" + isNegative(EmpCADTotal) + "</font></td></tr>";
		} 
		catch (e) 
		{
			content += "<br>format money issue..." + e.message + "<br>";
		}
	} //Employee For loop...
...
//		content+= "<td align='right' BGCOLOR='"+row_change(numOfEmployees) +"' title='Verbally Agreed Total is $" + SumVerbalTotal.formatMoney(0, '.', ',')  
//				+ "'><font size='2.4'>&nbsp;<u><b>$" + SumVerbalTotal.formatMoney(0, '.', ',')  + "</b></u></font></td>";
...
//		Altered Verbal Weighting
	...
	{
		content+= "<p>Error on #630: " + e.message + "</p>";
	}
