/************************************************************************
 * Name:		FirstPOS Library
 * Script Type:	FirstPOS Library
 *
...
var statusAwaitingProcessing = 1;
var statusCompleted = 2;
var statusFailed = 3;
var statusProcessed = 4;
 ...
 * 
 * version 1.1.0 - 24/01/2013 - adding replaceAll function - AS
 *****************************************************/
...
//.children(':last')
//.html(
//		jQuery('#phone_fs_lbl') 	// Field to add the button next to.
//		.parent()
//		.parent()
//		.children(':last')
//		.html() + '<input type="button" value="Call!" onclick="window.server_commands.src=\'\';">'
//);
...
/************************************************************************************
*
* rrunSavedSearches - checking the type of the data needs to output and 
* calling the functions accordingly 
* 
************************************************************************************/
function runSavedSearches(reportName,csvxml,type)
{
	if(csvxml == 'CSV')
	{
		//calling the runSavedSearch function
		runSavedSearch(0,1000,true, reportName,type);
	}
	else
	{
		// xml
		//calling the runSavedSearchXMLWithCriteria function
		runSavedSearchXML(0,1000,true, reportName, type);
	}
...
/************************************************************************************
 * runSavedSearch - Function that runs the saved search and make the results in
 * 					csv format
 *
 * @param from - starting record number
 * @param to	-ending record number
 * @param header - whether the header is needed or not
 * @param savedSearch - saved search name
 * @param type - saved search type (ex: item, treansaction)
 *
 * ************************************************************************************/

function runSavedSearch(from, to, header, savedSearch,type)
{
	var loadSearch = null;
	var runSearch = null;
	...
	//adding the first 1000 results after adding the header
	searchResults.forEach(function(srLine)
	{
		//calling the makeLine function to make the lines in csv format
...
/************************************************************************************
* runSavedSearchXML -Function that runs the saved search and make the results in XML format
...
************************************************************************************/
function runSavedSearchXML(from, to, header, savedSearch,type)
{
	var commaLine = '';
	var loadSearch = null;
	var runSearch = null;
	...
	//adding the first 1000 results
	searchResults.forEach(function(srLine)
	{
		//calling the makeXMLLine function to make the lines in XML format
...
/*******************************************************
 * 
 * makeHeader -Function that creating the header line of the CSV File's data
 * 
 * @param firstLine - the first line of the search results
 * @returns - header line
 * 
 ******************************************************/
...
/************************************************************
 * Function that creating the lines of XML File
 * 
 * @param srchRow
 * @returns {Array}
 * 
...
function makeXMLLine(srchRow)
{
	...
	// adding <record> tag in front of each line 
	line = '<record>';
	...
	cols.forEach(function(c)
	{
		//getting the each value's label name from the saved search results
		label = c.getLabel();
		
		//replacing the label name's spaces with '-'
		label = replaceAll(label,' ', '-');
		
		//adding the label name as the starting tag of each value
		xmlstart = '<'+ label + '>';
		
		//setting the ending tag for the value
		xmlend = '</'+ label + '>';
		
		//getting the value of the particular field
	...
	//ending the line with </record> tag
	line = line + '</record>';

	//calling the escapeCSV function to remove/escape particular signs/values
	line = escapeCSV(line);
...
/**************************************************
 * auditTrail function - creates an audit record and adds the CSV payload to it
 * 1.0.1 split out credit note number
 *********************************************************/
function auditTrail(message)
{

	var desc = 'FirstPOS Payload ' + Date();
	var payloadID = 0;
	var retVal = 'OK';
	var newPayload = null;
	...
	try
	{
		newPayload = nlapiCreateRecord('customrecord_firstpos_payload');
				
		newPayload.setFieldValue('name', desc);
		newPayload.setFieldValue('custrecord_firstpos_description', desc);
		newPayload.setFieldValue('custrecord_firstpos_payload', message);
		newPayload.setFieldValue('custrecord_firstpos_status', statusAwaitingProcessing);

		payloadID = nlapiSubmitRecord(newPayload, true);
