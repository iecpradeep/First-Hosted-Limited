 ...
 * for custom body field  for both new record and existing record
 * 
...
function calculateUnappliedAmountForOldRecord(receipt)
{
	...
	{	
		...
		for (var inv = 1; inv <= invoiceCount; inv++) 
		{
			invAmount = parseFloat(receipt.getLineItemValue('apply', 'amount', inv));
			invoiceTotal = invoiceTotal + invAmount;
		}
...
function setFieldValueForOldRecord(type)
{
	...
	{
		errorHandler("setFieldValueForCurrentRecord2 error", e);
	}
}
