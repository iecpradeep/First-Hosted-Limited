 ...
 * 			1.2.0 - 13/05/2013 - LD XML functions added - PAL
 *
 * Author:	FHL
...
function getXMLTree(theParent, theXML)
{
	...
	var currentElementInnerXML = '';
	var XMLremaining = theXML;
	var currentElementEndExp = null;
	var currentElementEnd = '';
	var thisElementBranch = new Array();
	...
	while (XMLremaining != '')
	{
		...
		currentElementEndExp = new RegExp(currentElement.replace(tagXMLstart, tagXMLstart + tagXMLterminator), "im");
		currentElementEnd = new String(currentElementEndExp.exec(XMLremaining));
		...
		if (currentElementExp.test(XMLremaining) && currentElementEndExp.test(XMLremaining))
		{
			...
			currentElementInnerXML = XMLremaining.substring(XMLremaining.indexOf(currentElement) + currentElement.length, XMLremaining.indexOf(currentElementEnd));

			thisElementBranch = new Array(removeTagCharacters(theParent), removeTagCharacters(currentElement), getXMLTree(currentElement, currentElementInnerXML), currentElementInnerXML);
...
function logXMLTree()
{
	...
	try
	{
		for (var t = 0; t < XMLtree.length; t++)
		{
			...
			XMLTreeHTML += "<tr><td>Node: " + t + "</td><td>" + XMLtree[t][0] + "</td><td>" + XMLtree[t][1] 	+ "</td><td>" + XMLtree[t][2] + "</td><td>" + "</td><td>" + TreeXMLorData + "</td></tr>";
		}

		XMLTreeHTML += "</table>";
		nlapiLogExecution('DEBUG', "XMLTree Order : " + orderNumber, XMLTreeHTML);
	}
	catch(e)
	{
		errorHandler('logXMLTree', e);
	}
}
 ...
 * removeTagCharacters
 * 
 * @param {String} theXMLtag
 * @returns {String} theXMLtag with XML tags removed
 * 
 ************************************************************************************/
...
function removeTagCharacters(theXMLtag) 
{
	...
	try
	{
		retVal = theXMLtag.replace(tagXMLstart,'').replace(tagXMLend,'').replace(tagXMLterminator,'');
	}
	catch(e)
	{
		errorHandler('removeTagCharacters', e);
	}
...
function createTaggedXML(theXMLtag, theInnerXML) 
{
	...
	try
	{
		retVal = tagXMLstartChar + theXMLtag + tagXMLendChar + theInnerXML + tagXMLstartChar + tagXMLterminator + theXMLtag + tagXMLendChar;
	}
	catch(e)
	{
		errorHandler('createTaggedXML', e);
	}
...
/************************************************************************************
 * 
 * Returns the data or XML content of first by default or nth occurrence of tree record that matches element name
 * 
 * @param {String} - theElementName
 * @param {String} - theIndex
 * @returns {String} - theData
 * 
 ************************************************************************************/
function getXMLTreeElementDatabyName(theElementName, theIndex) 
{
	...
	{
		errorHandler('getXMLTreeElementDatabyName', e);
	}
...
/************************************************************************************
 * 
 * Returns the first by default or nth occurrence of tree record that matches element name
 * 
 * 
 ************************************************************************************/
function getXMLTreeElementbyName(theElementName, theIndex) 
{
	...
	{
		errorHandler('getXMLTreeElementbyName', e);
	}
...
function getXMLTreeChildElementsbyName(theElementName) 
{
	...
	{
		errorHandler('getXMLTreeChildElementsbyName', e);
	}
...
function getXMLTreeChildNumberbyName(theElementName)
{
	...
	{
		errorHandler('getXMLTreeChildNumberbyName', e);
	}
