...
function matrixForm(request,response)
{
	...
	header += '<tr><td colspan=\"5\">' + lineCount +' items&nbsp;</td></tr>'
	...
	for (var i = 1; i <= lineCount; i++) 
	{
		...
		if (lineArray[i-1] == 0) 
		{
            ...
            if (parent != previousParent) {
				...
				// Throw a line gap if not the first ...
                if (previousParent != null) html += '<tr><td colspan=\"7\">&nbsp;</td></tr>';

				// Handle if first or another colour row found ...
                if (previousColour == null) previousColour = colour;
				if (nextColour == colour) nextColour = null;
		...
		for (sz=0; sz<numSizes; sz++) html += '<td align=\"right\">' + sizeArray[sz] + '[' + parent + ':' + colour + ']</td>';
