...
function genericSearch(table, fieldToSearch, valueToSearch)
{
	...
	//nlapiLogExecution('DEBUG', 'genericSearch', table + " : " +  fieldToSearch + " : " +  valueToSearch);
	...
	{
		//search filters                  
		searchFilters[0] = new nlobjSearchFilter(fieldToSearch, null, criteria,valueToSearch);                          
...
function getPalletSizeID(thePalletName){
	var theSizeID = null;
	if (thePalletName)
		theSizeID = genericSearch('customrecord_palletsize', 'custrecord_palletinputfield', thePalletName);	
	return theSizeID;
}
...
function getPURPrice(rate, currentForm, currentCustomer, parcelArea, consignmentWeight) {
	...
	if (getFormDeliveryType(currentForm) == 'TH') {
		...
		var PURcollservice = paramRecord
				.getFieldValue('custrecord_pur_collectservice');
		var PURbaseservice = paramRecord
				.getFieldValue('custrecord_pur_baseservice');
		var PURSurcharge = parseFloat(paramRecord
				.getFieldValue('custrecord_param_tpcollectionsurcharge'));
		var thSurcharge = 0.0;
		//1. See whether Customer has special collection rates for PURs
		var PURcustrate = getParcelPrice(currentCustomer, PURcollservice,
				parcelArea, consignmentWeight);
		if (PURcustrate == -1) { // Use base services rate for PURs
			if (isTestUser())
				alert('Base service = ' + PURbaseservice + ' area = '
						+ parcelArea + ' rate = ' + rate + ' Surcharge = '
						+ PURSurcharge);
			var PURbaserate = getParcelPrice(currentCustomer, PURbaseservice,
					parcelArea, consignmentWeight);
			...
			if (isTestUser())
				alert('Cust COLLrate T-H = ' + PURcustrate + ' area = '
						+ parcelArea + ' rate = ' + rate + ' Surcharge = '
						+ PURSurcharge);
			if (rate < PURcustrate)
...
/*
 * Post Code Lookup - Crafty Clicks
 */

function getNodeText(node){
	var returnText = '';
	//var node = xmlNodeDoc.getElementsByTagName(nodeName)[0];
	
	if (node){
		var nodeNode = node.childNodes[0];
		if (nodeNode && !(typeof nodeNode === "undefined")){
			returnText = nodeNode.nodeValue;
		}
	}
...
function XMLSoapLookup(pc)
{
	...
	if (pc != null && pc != '')
	{
		...
		params['key'] = "fcbb2-0bbe4-7f831-00e7b"; //UK Shuttle / Bryan Rose
		//params['key'] = "d2f14-e2ebe-70abe-00bf9"; //leighjdarby

		//var SOAPServiceURL = "https://pcls1.craftyclicks.co.uk/xml/rapidaddress?postcode=" + params['postcode'] + "&" + params['key'];
		var SOAPServiceURL = "https://pcls1.craftyclicks.co.uk/xml/rapidaddress?postcode=" + params['postcode'] + "&key=" + params['key'] + '&response=data_formatted';

		nlapiRequestURL( SOAPServiceURL, headers, null, handleResponse);
		function handleResponse(response) {
			var headers = response.getAllHeaders();
			//alert(response.getBody());
			//nlapiSetFieldValue('custbody_deliveryaddress', response.getBody());
			var xmlDoc = nlapiStringToXML(response.getBody().replace(
					/(\r\n|\n|\r)/gm, ""));
			var output = '';
			var townText = '';
			var countyText = '';
			...
			if (xmlDoc) {
				try {
					var deliverypoints = getNodeText(xmlDoc
							.getElementsByTagName("delivery_point_count")[0]);
					if (deliverypoints != '') {
						deliverypoints = parseInt(deliverypoints);

						countyText = getNodeText(xmlDoc
								.getElementsByTagName("postal_county")[0]);
						if (countyText == '')
							countyText = getNodeText(xmlDoc
									.getElementsByTagName("traditional_county")[0]);
						townText = getNodeText(xmlDoc
								.getElementsByTagName("town")[0]);

						output = deliverypoints + ' Addresses found for: ' + pc
								+ ', ' + townText + ', ' + countyText + '\n\n';
					
						var orgName = xmlDoc
						.getElementsByTagName("organisation_name");
						var bldgName = xmlDoc
						.getElementsByTagName("line_2");
						var bldgNumber = xmlDoc
								.getElementsByTagName("line_1");
						
						for ( var dpt = 0; dpt < bldgName.length; dpt++) {
							// alert(dpt);
							var bldgNumValue = getNodeText(bldgNumber[dpt]);
							var bldgValue = getNodeText(bldgName[dpt]);
							var orgValue = getNodeText(orgName[dpt]);
							output += (dpt + 1) + ' : ' + bldgNumValue
								+ ' : ' + bldgValue + ' : ' + orgValue + '\n';
						}
						output += '\n\nChoose a property from the list (1 - '
								+ bldgName.length + ')\n';
					} 
				} catch (e) {
					nlapiLogExecution('DEBUG', 'XMLSoapLookup',
							e.message);
					alert(e.message);
				}
			}
			...
			if (output != ''){
				var selectedAddrNo = prompt(output);
				if (!isNaN(selectedAddrNo)) {
					if (selectedAddrNo >= 1
							&& selectedAddrNo <= bldgName.length) {
						//alert("Selected Item :" + selectedAddrNo);
						// Assemble the new Address record
						try {
							newAddressObj = nlapiCreateRecord('customrecord_deliveryaddress');
							newAddressObj.setFieldValue('customform', 9);
							newAddressObj.setFieldValue(
									'custrecord_deladdress_custid',
									nlapiGetFieldValue('entity'));
							
							var formattedPC = checkPostCode(pc);
							if (formattedPC)
								pc = formattedPC;							

							var delAddress1 = getNodeText(bldgNumber[selectedAddrNo - 1]);
							if (delAddress1 == '')
								delAddress1 = 'na';
							newAddressObj.setFieldValue(
									'custrecord_deladdress_addr1', delAddress1);

							newAddressObj.setFieldValue(
									'custrecord_deladdress_city', townText);
							newAddressObj.setFieldValue(
									'custrecord_deladdress_county', countyText);
							
							newAddressObj.setFieldValue(
									'custrecord_deladdress_postcode', pc);
							newAddressObj.setFieldValue(
									'custrecord_deladdresstelno', 'na');
							newAddressObj.setFieldValue(
									'custrecord_contactname', 'na');

							var delAddressName = '';
							var orgRefNode = orgName[selectedAddrNo - 1];
							
							var orgRefNodeText = '';
							if (orgRefNode != '' && orgRefNode != null)
								orgRefNodeText = getNodeText(orgRefNode);

							//alert('orgRefNodText: ' + orgRefNodeText);

							if (orgRefNodeText != '' && orgRefNodeText != null)
								delAddressName = orgRefNodeText;	
							
							while (delAddressName == '' || delAddressName == null){
								delAddressName = prompt('Selected address :'  + delAddress1 + ' '+ countyText + ' '+ townText + '\n\nPlease give a business / contact name : ');
								//delAddressName = 'na';
							}							
							newAddressObj
									.setFieldValue('custrecord_deladdressname',
											delAddressName);

							newAddressObj.setFieldValue('name', delAddressName
									+ ", " + pc);

							var returnAddressID = nlapiSubmitRecord(newAddressObj, false);
							if (returnAddressID){
								//alert("New address created - ID : " + returnAddressID);
								nlapiSetFieldValue('custbody_deliveryaddressselect', returnAddressID, true, true);
								pcFound = true;
							}								
				...
				alert('No delivery points found for postcode : ' + pc);
			}						
		}
