...
if (request.getParameter('custparam_manifestdate') != '') theDate = request.getParameter('custparam_manifestdate');
var dateArray = theDate.split("/");
var theLongDate = getLongDay(dateArray[0]) + " " + getLongMonth(dateArray[1]) + " " + dateArray[2] ;

function createReportHeader(fromDate,toDate,showAll){

	var reportTitle = 'NEP Parcels';
    ...
    var header = '<tr><td colspan=\"11\" align=\"center\"><h1>' + reportTitle + '</h1></td></tr>';
    header += '<tr><td colspan=\"6\" ' + fontStyle1 + '><h1>Delivery Manifest</h1></td><td>&nbsp;</td><td colspan=\"5\" ' + fontStyle1 + ' align=\"right\"><h1>' + dateRange + '</h1></td></tr>';
    header += '<tr><td colspan=\"6\" ' + fontStyle1 + '><b>' + nlapiEscapeXML(companyAddr).replace(/\n/g, '<br />') + '</b></td><td>&nbsp;</td><td colspan=\"5\" ' + fontStyle1 + ' align=\"right\">Account:&nbsp;' + theAcct + '</td></tr>';
    header += '<tr><td colspan=\"12\">&nbsp;</td></tr>';
    header += '<tr><td>DOCNUM</td><td>SVCE</td><td>CONSIGNEE</td><td>REF.</td><td>TOWN</td><td width="15mm">PCODE</td><td>ITEMS</td><td>[CHK]</td><td>VWT</td><td>SPCL</td><td>CHARGE</td><td align="right">WT</td></tr>';
    header += '<tr><td colspan=\"12\" style=\"border-top:solid black 2px;\">&nbsp;</td></tr>';
    return header;
}
...
function createReportFooter(totalConsignments, totalCartons, totalWeight){
    ...
    var footer = '<tr><td ' + fontStyle5 + ' align=\"right\" colspan=\"12\">SPCL Key: F = Fragile, S = Security</td></tr>';
    ...
    footer += '<tr><td align=\"right\" colspan=\"5\"><b>Total Weight</b></td><td ' + fontStyle2 + ' align=\"right\"><b>&nbsp;' + totalWeight.toFixed(2) + '</b></td></tr>';
    footer += '<tr><td colspan=\"6\">&nbsp;</td></tr>';
    footer += '<tr><td ' + fontStyle5 + ' colspan=\"12\">Confidential Information.&nbsp;&copy;&nbsp;All rights reserved.</td></tr>';
    return footer;
}
...
function printManifest(request, response){ // no paramater = default to todays's date ...
    ...
    html += createReportHeader(fromDate,toDate, showAll);
    ...
    mySearchFilters[4] = new nlobjSearchFilter('custbody_palletparcel', null, 'anyof', ('1')); // Parcels only
    mySearchFilters[5] = new nlobjSearchFilter('class', null, 'noneof', ('6')); // Not PURs
	if (showAll) {
    ...
    mySearchColumns[9] = new nlobjSearchColumn('custbody_insurancesurcharge'); 
    ...
    if (mySearchResults) {
        ...
        for (i = 0; i < mySearchResults.length; i++) {
            ...
            var service = mySearchResults[i].getValue(mySearchColumns[1]);
            ...
            var charge = '';
			var inscharge = mySearchResults[i].getValue(mySearchColumns[9]);
			if (!isNaN(inscharge))
			 if (inscharge > 0) charge = parseFloat(inscharge).toFixed(2);
			
            html += '<tr><td>' + docNum + '</td><td>' + service + '</td><td align="left" width="15mm">' + consignee + '</td><td>' + custRef + '</td><td>' + town + '</td><td style="width:15mm;">' + postcode + '</td><td>' + parcels + '</td><td>[&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;]</td><td align=\"right\">&nbsp;</td><td>' + fswflag + '</td><td align=\"right\">' + charge + '</td><td align="right">' + parcelweight.toFixed(2) + '</td></tr>';
        } // for ...
        html += createReportFooter(mySearchResults.length, totalCartons, totalWeight);
    }
    else {
        html += '<tr><td colspan=\"12\">There are no consignments for ' + theLongDate + '</td></tr>';
    }
