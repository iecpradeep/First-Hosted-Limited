function consignmentManager(request, response){
    ...
    var form = nlapiCreateForm('Consignment Manager (v3.1h)');
	...
	//form.addField('schedule','label','Quick Date : ').setLayoutType('startcol');
	
	//adding radio buttons
	var noDates = form.addField('daterange', 'radio', 'Ignore Dates', 'nodates');	
	var scheduleDaily = form.addField('daterange', 'radio', 'Last 2 Days', 'last2days');
	var scheduleWeekly = form.addField('daterange', 'radio', 'Last Week','lastweek');
	var scheduleMonthly = form.addField('daterange', 'radio', 'Last Month','lastmonth');
	//setting the radio button's default value
	form.getField('daterange', 'nodates' ).setDefaultValue( 'nodates' );		
	*/
    ...
    if (useCustomerService) {
        ...
        var customerField = form.addField('custpage_customerfield', 'text', 'NEP Customer Name');
        ...
        var customerSelectField = form.addField('entityid', 'select', 'NEP Account Ref.', 'customer');
        if (custID > 0) {
    ...
    */
    ...
    if (extdocref != '' && extdocref != null) {
		...
		var theExtIDs = 'APC,TPN01,TPN02,LOCAL,APCINV,TPN01INV,TPN02INV,LOCALINV,LOCALDEL,APCDEL,TPN01DEL,LDL'.split(',');
    ...
    if (ignoredates != 'T'  && !noConsignmemnts) {
        ...
        if (searchParams == '') { // Cannot allow a search of all consignments - need to provide >=1 search parameters
			consignmentFilters[n] = new nlobjSearchFilter('trandate', null, 'onorafter', '31/12/2040'); // i.e. no jobs listed
            n++;
        }
    }
    ...
    if (isTestUser()){      
    	...
    	testField.setDefaultValue(searchParams + '<br />ExtIDs=' + testExtIDs);
    }
    ...
    if (consignmentResults) {
        ...
        if (rowLimit < maxRowsAllowed) {
            ...
            var printScriptVersion = '22'; // Parcel label script - NEW VERSION
            if (isTestUser())
            	printScriptVersion = '65';
            var printScriptPalletVersion = '19'; // Pallet label script - NEW VERSION
            var printScriptOtherVersion = '60'; // for RCCs etc.
            var printScriptOtherDeliveryNote = '56'; // for RCCs etc.
            var updateScriptVersion = '63';
            var convertScriptVersion = '64';
            ...
            */
            ...
            for (var i = rowStart; i < rowEnd; i++) {
                ...
                if (i < rowEnd) {
                    ...
                    var editURL = "&nbsp;";
                    var editURLMaster = "<a href='scriptlet.nl?script=14&deploy=1" + versionParam + "&custparam_tranid=" + consignmentResults[i].getId() + "&custparam_cf=" + formID + "' target='_blank'>Edit</a>";
...
function isAdvancedSearch(){
    ...
    //if (theRole == 1009) 
    if (theRole == 3 || theRole == 14 || theRole == 1001 || theRole == 1009 || theRole == 1015 || theRole == 1010  || theUser == 7 || theUser == 777 || isTransport()) 
        isSearch = true;
    return isSearch;
}
...
function isCustomerCenter(){
    var isCenter = false;
    var theRole = parseInt(nlapiGetContext().getRole());
    if (theRole == 14 || theRole == 1001 || theRole == 1005 || theRole == 1006 || theRole == 1007 || theRole == 1008 || theRole == 1015) 
        isCenter = true;
    return isCenter;
}
