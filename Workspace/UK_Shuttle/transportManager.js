function transportManager(request, response){
    var form = nlapiCreateForm('Transport Manager (v3.1d)');
    var rowLimit = 2000;
    ...
    var classselectColumns = new Array;
    var classselectFilters = new Array;

    classselectColumns[0] = new nlobjSearchColumn('internalid');
    classselectColumns[1] = new nlobjSearchColumn('name');
	classselectColumns[2] = new nlobjSearchColumn('custrecord_class_seqno');
	classselectColumns[3] = classselectColumns[2].setSort();
 	classselectColumns[4] = new nlobjSearchColumn('custrecord_class_palletservice');
 	classselectColumns[5] = new nlobjSearchColumn('custrecord_class_parcelservice');
 	classselectColumns[6] = new nlobjSearchColumn('custrecord_class_collectservice');
 	classselectColumns[7] = new nlobjSearchColumn('custrecord_class_deliveryservice');
 
	var classselectResults = nlapiSearchRecord('classification', null, classselectFilters, classselectColumns);
	...
	for (var cl=0; cl<classselectResults.length; cl++){		
		var classselected = false;
		if (classselectResults[cl].getValue(classselectColumns[0]) == classselect) classselected = true;
		classselectField.addSelectOption(classselectResults[cl].getValue(classselectColumns[0]), classselectResults[cl].getValue(classselectColumns[1]), classselected);
	}
    ...
    var closeTimeField = 'custentity_closetime';
    if (d.getDay() == 5) // today is Friday
	   closeTimeField = 'custentity_fridayclosetime';
	...
	var sortselect = request.getParameter('custpage_sortselect');
	if (sortselect == null || sortselect == '')
		sortselect = 0; // Default
    var sortselectField = form.addField('custpage_sortselect', 'select', 'Sort by ');
	var sortArray = new Array('Sort by Most Recent', 'Sort by Name');
    var selectedVal = '';
    for (var ec = 0; ec < sortArray.length; ec++) {
        var selectThis = false;
        if (sortselect == ec) {
            selectThis = true;
        }
        sortselectField.addSelectOption(ec, sortArray[ec], selectThis);
    }
    ...
    if ((now.getMonth() >= 4 || (now.getMonth() == 3 && now.getDate() > 27)) && (now.getMonth() <= 11 || (now.getMonth() == 10 && now.getDate() < 28))) 
        nowHours += 1; //Daylight savings approximation!
	nowMinutes = now.getMinutes();
	if (nowMinutes < 10)
		nowMinutes = '0' + nowMinutes;

	var DateTimeField = form.addField('lastrefresh', 'text', 'Last Refresh: ');
    DateTimeField.setDisplayType('inline');
    DateTimeField.setDefaultValue(nowHours + ":" + nowMinutes);

/*	var testData = '';
	var testField = form.addField('testalert', 'text', 'Test Data:');
    testField.setDisplayType('inline');
    testField.setDefaultValue(testData);

	var test4Data = '';
	var test4Field = form.addField('test4alert', 'text', 'Test Data:');
    test4Field.setDisplayType('inline');
    test4Field.setDefaultValue(test4Data);

	var test2Data = '';
	var test2Field = form.addField('test2alert', 'text', 'Test 2 Data:');
    test2Field.setDisplayType('inline');
    test2Field.setDefaultValue(test2Data);

	var test3Data = '';
	var test3Field = form.addField('test3alert', 'text', 'Test 3 Data:');
    test3Field.setDisplayType('inline');
    test3Field.setDefaultValue(test3Data);
*/
	...
	*/
	...
	form.addButton('custpage_back', 'Consignment Manager', "window.open('" + nlapiResolveURL('SUITELET', thisCMscript, thisCMdeployment) + versionParam + cleardtParam + "','_self')");
    ...
    form.addButton('custpage_tpnservicesummary', 'TPN by Service Summary ...', "window.open('../../../app/common/search/searchresults.nl?searchid=111','_blank')");
...
//if (currentClass == 7){
	...
	consignmentFilters[2] = new nlobjSearchFilter('custentity_excludefrom_tm', 'customer', 'is', 'F');
	
	var nextFilter = 3;
	if (currentClass == 7) {
		consignmentFilters[3] = new nlobjSearchFilter('entity', null, 'noneof', custAPCDELIVERY);
		consignmentFilters[4] = new nlobjSearchFilter('entity', null, 'noneof', custTPNDELIVERY);
		nextFilter = 5;
	}
	...
	if (useTotals) {
		...
		consignmentColumns[15] = new nlobjSearchColumn('datecreated');
		if (isDeliverService) {
			...
			consignmentColumns[20] = new nlobjSearchColumn('custbody_deliverypostcode');
			consignmentColumns[21] = consignmentColumns[3].setSort();
		} else {			
			consignmentColumns[16] = new nlobjSearchColumn('companyname', 'customer');
			consignmentColumns[17] = new nlobjSearchColumn('custentity_dailycollection', 'customer');
			consignmentColumns[18] = new nlobjSearchColumn('custentity_maxparcels_alert', 'customer'); 
			consignmentColumns[19] = new nlobjSearchColumn(closeTimeField, 'customer'); 
			//consignmentColumns[20] = new nlobjSearchColumn('custentity_fridayclosetime', 'customer'); 
			var sortCol = 0;
			//if (sortselect == 0) sortCol = 15;
			consignmentColumns[20] = consignmentColumns[sortCol].setSort();
		}
	}
	
	//if (isDeliverService) {
	if (currentClass == 8 && !useTotals) { // APC Delivery Class
		var consignmentResults = nlapiSearchRecord('salesorder', 'customsearch_alldailytransactions_4', consignmentFilters, consignmentColumns);
	}
	else {
    ...
    if (consignmentResults) {
		...
		var sortConsignmentArray = new Array(consignmentResults.length);
		var sortDateTimeArray = new Array(consignmentResults.length);

		for (var i = rowStart; i < rowEnd; i++) {
			sortConsignmentArray[i-rowStart] = new Array(4);
			sortConsignmentArray[i-rowStart][0] = consignmentResults[i].getValue(consignmentColumns[0]); // Entity ID
			sortConsignmentArray[i-rowStart][1] = consignmentResults[i].getValue(consignmentColumns[1]); // Tran ID	
			sortConsignmentArray[i-rowStart][2] = Date.parse(consignmentResults[i].getValue(consignmentColumns[15])); // Time created turned into integer milliseconds since Jan 1 1970
			sortConsignmentArray[i-rowStart][3] = i; // The index to the search row data for later
			sortDateTimeArray [i-rowStart] = parseFloat(sortConsignmentArray[i-rowStart][2] + '.' + sortConsignmentArray[i-rowStart][0]) + 0.0009; // Add a 9 to prevent any trailing 0's dropping off the entity id
        	//nlapiLogExecution('AUDIT', 'ArrayLoad: Row ' + (i-rowStart), "NAME:" + sortConsignmentArray[i-rowStart][0] + " TRANID:" + sortConsignmentArray[i-rowStart][1] + " CREATED:" + sortConsignmentArray[i-rowStart][2]);
		}
		      
        if (sortselect == 0) { // Sort by most recently created first, then by entityid
            sortDateTimeArray.sort(function(a,b){return b-a});			
        }
        
		var entityidList = '';
        for (var i = 0; i < sortDateTimeArray.length; i++) {
			//nlapiLogExecution('AUDIT', 'ArraySort: Row ' + i,sortDateTimeArray[i]);
			for (var j = 0; j < sortDateTimeArray.length; j++) {
				if (sortDateTimeArray[i] == parseFloat(sortConsignmentArray[j][2] + '.' + sortConsignmentArray[j][0]) + 0.0009) {
					//nlapiLogExecution('AUDIT', 'ArraySort Match: Row ' + j, "NAME:" + sortConsignmentArray[j][0] + " TRANID:" + sortConsignmentArray[j][1] + " CREATED:" + sortConsignmentArray[j][2]);
					if (entityidList == '') {
						entityidList = sortConsignmentArray[j][0];
					} else {
						var currentidList = entityidList.split(',');
						var idFound = false;
						for (var id = 0; id < currentidList.length; id++){
							if (currentidList[id] == sortConsignmentArray[j][0])idFound = true;
						}
						if (!idFound) entityidList += ',' + sortConsignmentArray[j][0];
					}
				}				
			}
		}
		var entityidArray = entityidList.split(','); // The correctly sorted list of customers
		var currentEntityID = 0;
		var sortXrefArray = new Array(sortDateTimeArray.length);
		var currentXref = 0;
		for (var entityref = 0; entityref < entityidArray.length; entityref++) {
			currentEntityID = entityidArray[entityref];
			for (var xref = 0; xref < sortDateTimeArray.length; xref++) {
				if (sortConsignmentArray[xref][0] == currentEntityID) { // Add to xref
					sortXrefArray[currentXref] = new Array(2);
					sortXrefArray[currentXref][0] = currentEntityID;
					sortXrefArray[currentXref][1] = sortConsignmentArray[xref][3];	// the 'i' index to the search record				
					//nlapiLogExecution('AUDIT', 'ArrayXref Match: Row ' + xref, "ENTITYID:" + sortXrefArray[currentXref][0] + " SEARCH(i):" + sortXrefArray[currentXref][1]);
					currentXref++;
				}
			}
		}
		var zeroIndex = sortXrefArray[0][1]; // Used below for the setup of first row / fields
        ...
        if (isDeliverService && !useTotals && showallconsignments == 'T') {
            sublist.addField('custpage_' + c + '_col_driverselect', 'select', 'Driver Allocation', 'customrecord_driverrun');
        }
        
        if (!isDeliverService)
        	sublist.addField('custpage_' + c + '_col_closetime', 'text', 'Closes');
        ...
        if (isParcelService) {
            ...
            sublist.addField('custpage_' + c + '_col_parcelspurs', 'text', 'PUR Parcels');
        ...
        if (isPalletService) {
            ...
            sublist.addField('custpage_' + c + '_col_palletspurs', 'text', 'PUR Pallets');
        ...
        //sublist.setLineItemValue('custpage_' + c + '_col_index', 1, '<b>' + rowStart + "/" + rowEnd + '</b>');
        ...
        if (isParcelService) {
            ...
            sublist.setLineItemValue('custpage_' + c + '_col_parcelspurs', 1, '<b>' + grandtotalPURParcels + '</b>');
        ...
        if (isPalletService) {
            ...
            sublist.setLineItemValue('custpage_' + c + '_col_palletspurs', 1, '<b>' + grandtotalPURPallets + '</b>');
		...
		var maxParcelsAlert = 1000;
		
        if (useTotals) {
            var thisCustomer = consignmentResults[zeroIndex].getValue('name', null, 'group'); // Set to the first customer id
            var thisDriverRun = consignmentResults[zeroIndex].getValue('custbody_driverrun', null, 'group'); // Set to the first driver id
            var thisDriverRunText = consignmentResults[zeroIndex].getText('custbody_driverrun', null, 'group'); // Set to the first driver id
            var thisCustomerText = consignmentResults[zeroIndex].getText('name', null, 'group'); // Set to the first customer text
			//thisCustomerText += " (" + statusClass + "/" + PURConsignments + ")";
        }
		else {
			var thisCustomer = consignmentResults[zeroIndex].getValue(consignmentColumns[0]); // Set to the first customer id
			var thisDriverRun = consignmentResults[zeroIndex].getValue(consignmentColumns[3]); // Set to the first driver id
			var thisDriverRunText = consignmentResults[zeroIndex].getText(consignmentColumns[3]); // Set to the first driver id
			var thisCustomerText = consignmentResults[zeroIndex].getText(consignmentColumns[0]); // Set to the first customer text
		}
		...
		var thisCustomerHTML = consignmentResults[zeroIndex].getValue(consignmentColumns[16]);
		
		//if (isDeliverService)
		//	thisCustomerHTML = consignmentResults[zeroIndex].getText(consignmentColumns[3]);
		//thisCustomerHTML += " (" + i + ")";

        //var thisCustomerDailyCollect = custData.custentity_dailycollection;
        var thisCustomerDailyCollect = consignmentResults[zeroIndex].getValue(consignmentColumns[17]);
        var thisCustomerCloseTime = consignmentResults[zeroIndex].getValue(consignmentColumns[19]);        
        ...
        if (thisCustomer == custAPCDELIVERY || thisCustomer == custTPNDELIVERY) {
        	thisCustomerHTML = " Driver " + thisDriverRunText; 
        } else {
            ...
            //for (var i = rowStart; i < rowEnd; i++) {
			rowStart = 0;
			rowEnd = sortXrefArray.length;
            for (var thisRef = rowStart; thisRef <= rowEnd; thisRef++) {
            	//if (i == rowEnd-1) {
                
                if (parseInt(thisRef) == parseInt(rowEnd)) {
					displaySummary = true;
				}
				else {
					var i = sortXrefArray[thisRef-rowStart][1];
					if (useTotals) {
			...
			//nlapiLogExecution('AUDIT', 'ArrayXref Match: Row ' + (thisRef-rowStart) + ": ThisC:" + thisCustomer + ": CurrC:"  + currentCustomer, "rowStart: " + rowStart + " rowEnd: " + rowEnd  + " thisRef: " + thisRef + " ==> i: " + i + " displaySummary? " + displaySummary);	
			
			//testData += i + "|";
			//test2Data += currentCustomer.slice(0,1) + "|";
			//test2Data += currentCustomer + "|";
			//var display = 'T';
			//if (!displaySummary) display = 'F';
			//test3Data += display + "| ";
			//test4Data += (lineNum - 1).toFixed(0) + "| ";			
            ...
            if (displaySummary) {
                ...
                if ((readyConsignments == processedConsignments && driverList != '') || readyConsignments == 0) {
                    if (totalParcels > maxParcelsAlert) {
                        colorStatus = colorUrgentStatus;
                    }
                    else {
                ...
                var parcelsAlert = false;
                
                if ((readyParcelConsignments == EPODCollectedParcelConsignments && driverList != '' && readyParcelConsignments != 0)) {
                	colorParcelStatus = colorOKStatus;
                }
                else {
                	if (totalParcels > maxParcelsAlert) {
                		parcelsAlert = true;
                		colorParcelStatus = colorUrgentStatus;
                	} else {
                 ...
                 */
                //if ((readyPalletConsignments == (processedPalletConsignments + printedPalletConsignments) && driverListPallets != '') || readyPalletConsignments == 0) {
                ...
                searchListURL = "<a href='" + CMURL + "' target='_blank'>";
                
                var totalCollectDeliver = 0;
				var parcelAlertText = '';
				if (totalParcels > maxParcelsAlert)
					parcelAlertText = ' (+' + (totalParcels-maxParcelsAlert)*1 + ')';
                sublist.setLineItemValue('custpage_' + c + '_col_seqno', lineNum, (lineNum - 1).toFixed(0));
                //sublist.setLineItemValue('custpage_' + c + '_col_index', lineNum, thisCustomerText + " (" + PURParcelConsignments + "," + PURPalletConsignments + "/" + totalPURParcels + "," + totalPURPallets + ")");
                sublist.setLineItemValue('custpage_' + c + '_col_index', lineNum, thisCustomerText);
                if (!isDeliverService)
                    sublist.setLineItemValue('custpage_' + c + '_col_closetime', lineNum, thisCustomerCloseTime);
                	
                if (isParcelService) {
                    sublist.setLineItemValue('custpage_' + c + '_col_drivers', lineNum, driverList);
                    if ((parseInt(totalParcelConsignments) > 0 || totalCancelledParcels > 0) || isDeliverService) 
                        sublist.setLineItemValue('custpage_' + c + '_col_parcelconsignmentno', lineNum, searchListURL.slice(0,256).replace("*CT*", "1") + '<span style=\"background-color:' + colorParcelStatus + '\">' + totalParcelConsignments + '-' + thisCustomerHTML + '</span></a>');
                    ...
                    sublist.setLineItemValue('custpage_' + c + '_col_parcels', lineNum, '<b>' + totalParcels + parcelAlertText + '</b>');
                    sublist.setLineItemValue('custpage_' + c + '_col_parcelsweight', lineNum, '<b>' + totalParcelsWeight + ' KG</b>');
                    sublist.setLineItemValue('custpage_' + c + '_col_parcelspurs', lineNum, totalPURParcels.toFixed(0));
                ...
                if (isPalletService) {
                    if (isDeliverService) {
                        ...
                        //searchListURL = "<a href=''>";
                        thisCustomerHTML = thisCustomerHTML.replace('Driver Delivery Run Pallets - ', '');
                    }
                    else {
                    ...
                    if ((parseInt(totalPalletConsignments) > 0 || totalCancelledPallets > 0) || isDeliverService) 
                        sublist.setLineItemValue('custpage_' + c + '_col_palletconsignmentno', lineNum, searchListURL.replace("*CT*", "2") + '<span style=\"background-color:' + colorPalletStatus + '\">' + totalPalletConsignments + '/' + totalCollectDeliver + '&nbsp;-&nbsp;' + thisCustomerHTML + '</span></a>');
                    ...
                    sublist.setLineItemValue('custpage_' + c + '_col_palletspurs', lineNum, totalPURPallets.toFixed(0));
                ...
                //Update grand totals
                if (!isNaN(totalParcels)) 
                    grandtotalParcels += totalParcels;
                if (!isNaN(totalCancelledParcels)) 
                    grandtotalCancelledParcels += totalCancelledParcels;
                if (!isNaN(totalPURParcels)) 
                    grandtotalPURParcels += totalPURParcels;
                if (!isNaN(totalParcelsWeight)) 
                    grandtotalParcelsWeight += totalParcelsWeight;
                if (!isNaN(totalPallets)) 
                    grandtotalPallets += totalPallets;
                if (!isNaN(totalCancelledPallets)) 
                    grandtotalCancelledPallets += totalCancelledPallets;
                if (!isNaN(totalPURPallets)) 
                    grandtotalPURPallets += totalPURPallets;
                if (!isNaN(totalPalletsWeight)) 
                    grandtotalPalletsWeight += totalPalletsWeight;
                if (!isNaN(totalConsignments)) 
                    grandtotalConsignments += totalConsignments;
                if (!isNaN(totalParcelConsignments)) 
                    grandtotalParcelConsignments += totalParcelConsignments;
                if (!isNaN(totalPalletConsignments)) 
                    grandtotalPalletConsignments += totalPalletConsignments;
                
                //if (i != rowLimit) {                    
                if (thisRef != rowEnd && thisRef != rowLimit) {
                    ...
                    thisCustomerHTML = consignmentResults[i].getValue(consignmentColumns[16]);
                    //thisCustomerHTML += " (" + i + ")";
                    //thisCustomerDailyCollect = nlapiLookupField('customer', thisCustomer, 'custentity_dailycollection', false);
                    thisCustomerDailyCollect = consignmentResults[i].getValue(consignmentColumns[17]);
                    thisCustomerCloseTime = consignmentResults[i].getValue(consignmentColumns[19]);
                    if (thisCustomer == custAPCDELIVERY || thisCustomer == custTPNDELIVERY) 
                        thisCustomerHTML = " Driver Delivery Run " + thisDriverRunText;
                    for (h = 0; h < (70 - thisCustomerHTML.length); h++) 
                        thisCustomerHTML += '&nbsp;' //Pad with spaces
                    maxParcelsAlert = 1000;
                    if (consignmentResults[i].getValue(consignmentColumns[18]) != '' && consignmentResults[i].getValue(consignmentColumns[18]) != null) 
                        maxParcelsAlert = parseInt(consignmentResults[i].getValue(consignmentColumns[18]));
                    totalParcels = 0;
                    ...
                    enteredParcelConsignments = 0;
                    enteredPalletConsignments = 0;
                    printedConsignments = 0;
					printedParcelConsignments = 0;
					printedPalletConsignments = 0;
                    processedConsignments = 0;
                    processedParcelConsignments = 0;
                    processedPalletConsignments = 0;
                    ...
                    EPODCollectedParcelConsignments = 0;
                    EPODDeliveredParcelConsignments = 0;
                    EPODAcceptedParcelConsignments = 0;
            ...
            if (thisRef != rowEnd && thisRef != rowLimit) {                    
				...
				if (statusClass == '6' || statusClass == '10'){ // PUR Classes
					...
					if (statusClass == '6') PURParcelConsignments += statusSum;
					if (statusClass == '10') PURPalletConsignments += statusSum;
				}
                     ...
                     if (showallconsignments == 'T') {
                        ...
                        sublist.setLineItemValue('custpage_' + c + '_col_index', lineNum, consignmentResults[i].getText(consignmentColumns[0]) + " - " + externalRefNo);
                         ...
                         if (isPalletService) {
                             ...
                             sublist.setLineItemValue('custpage_' + c + '_col_deliverpostcode', lineNum, consignmentResults[i].getValue(consignmentColumns[20]));						
		...
		/*
		testField.setDefaultValue(testData.slice(0,300));
		test2Field.setDefaultValue(test2Data.slice(0,300));
		test3Field.setDefaultValue(test3Data.slice(0,300));
		test4Field.setDefaultValue(test4Data.slice(0,300));
		*/

        //if (isPalletService && isCollectService && parseInt(totalPalletConsignments) > 0) {
        if (isPalletService && isCollectService) {
 ...
 if (versionParam == "&version=2")
 	nlapiLogExecution('AUDIT', 'Transport_Manager:' + classDateselect , "SORTBY:" + sortselect + " IDLIST:" + entityidList  + " REMAINING:" + theContext.getRemainingUsage());
...
function isCustomerCenter(){
	var isCenter = false;
	if (parseInt(nlapiGetContext().getRole()) == 1001 || parseInt(nlapiGetContext().getRole()) == 1015 || parseInt(nlapiGetContext().getRole()) == 1005 || parseInt(nlapiGetContext().getRole()) == 1006) isCenter = true;
	return isCenter;
}
