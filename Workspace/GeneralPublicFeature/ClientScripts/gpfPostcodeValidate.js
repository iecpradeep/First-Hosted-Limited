...
function validateCountryPostcode(type)
{
	...
	if (shipPostCode && shipCountry) 
	{
		if (isValidPostcode(shipPostCode, shipCountry)) 
		{
			...
			if (nlapiGetFieldValue('custbody_palletparcel') == '2') 
			{ //Pallet lookup
...
function getPostcodeSurcharge(postCode, showAlert, items)
{
	...
	if (searchCode) 
	{
		...
		if (postcodeSearchResults) 
		{
			...
			if (showAlert) 
			{
				alert ( "Please Note: there is a surcharge for post code " + searchCode + " owing to 2012 Olympics.\n\nFrom " + fromDate + " to " + toDate + "\n\nReason: " + desc + "\n\n" + packageAlert+ "\n\nTotal (GBP) : " + surcharge);
			}
		}
	}
	return surcharge;
