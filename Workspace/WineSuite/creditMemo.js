...
var afterSubmit = function(type){
	...
	else if(type == 'edit')
	{
		...
		if(recordType == 'creditmemo')
		{
			...
			nlapiLogExecution('debug', 'IN EDIT AFTER: isBlank(createdfrom)');
			...
			if(count > 0)
			{
				for(var i = 1; i <= count; i++)
				{
					...
					nlapiLogExecution('debug', 'rotationID', rotationID);
					
					if(!isBlank(rotationID))
					{
						try
						{
							//[TODO] - copyRotation is out of scope
