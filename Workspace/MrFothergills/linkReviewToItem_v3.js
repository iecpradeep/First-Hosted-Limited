/*****************************************************************************
 * Name:		linkReviewToItem_v3.js
 * Script Type:	UserEvent
 * Client: 		Mr Fothergill's Seeds Limited
 *
 * Version:		1.0.0 - 06/04/2012 - First Release - GP
 *				1.0.1 - 10/12/2012 - Check if title is null - SB
 *				1.0.2 - 11/12/2012 - Add star rating - SB
 *
 * Author:		GProxy (from eCommerce Bundle)
 * Purpose:		Links Product Review to Item
 * 
 * Script: 		customscript1
...
function saveReview(type) {
	...
	var itemId;
		...
		for(var i = 1; i <= 5; i++)
		{
			if (i <= rating)
			{
				ratingHTML += "<img src='/c.3322617/site/common/img/1.gif' alt='*'> ";
			}
			else
			{
				ratingHTML += "<img src='/c.3322617/site/common/img/0.gif' alt='-'> ";
			}
		}
		
		//reviewerRating = '<b>Rating:</b> ' + rating + '<br><br>'; // 1.0.2
		reviewerRating = '<b>Rating:</b> ' + ratingHTML + '<br><br>'; // 1.0.2
		reviewerTitle = '';
		if (reviewerTitle)
		{
			reviewerTitle = '<b>' + newRecord.getFieldValue('custrecord_productreview_yourtitle')  + '</b><br>';
		}
		reviewerBody = newRecord.getFieldValue('custrecord_productreview_yourreview') + '<br><br><br><br>';
			...
			// Clear the product reviews we may have in memory.
				...
				if (reviewresults != null) 
				{
					for(var count = 0; count < reviewresults.length; count++) {
						...
						//var reviewRating = '<b>Customer Rating:</b> ' + rating + '<br><br>'; // 1.0.2
						var reviewRating = '<b>Customer Rating:</b> ' + ratingHTML + '<br><br>'; // 1.0.2
	     	  				var reviewTitle = '';
	     	  				if (reviewerTitle)
	     	  				{
	     	  					reviewTitle = '<b>' + reviewRecord.getFieldValue('custrecord_productreview_yourtitle') + '</b><br>';
	     	  				}

	     	  				var reviewBody = reviewRecord.getFieldValue('custrecord_productreview_yourreview') + '<br><br><br><br>';	
