 ...
 * 					1.0.1 - 07/03/2013 - Altered the form from pdfxml to 
 * Author: 			FHL
...
//		shipfromfld = form.addField('custpage_shipfrom','textarea', 'Warehouse Address').setDefaultValue(shipaddress);
		...
		deldatecol = polines.addField('custbody_lhkreceiveby','date', 'Delivery Date');
...
//		Search the Purchase Order Record for columns to print and filters to exclude items which have been received
		...
		deldatecol = new nlobjSearchColumn('custbody_lhkreceiveby',null,null);
...
//		Write the Form
	...
	else
	{
		...
		pdfxml += '<p><img display="inline" src="http://shopping.netsuite.com/c.1336541/site/images/LHK-Supplierpackinglist.PNG" dpi="100" /></p>';
...
//				pdfxml += constructXML(request.getLineItemValue('sublist_polines','tranid',lineNum));
				...
				deldateval = request.getLineItemValue('sublist_polines','custbody_lhkreceiveby',lineNum);
				if (deldateval == null)
				{
					deldateval = '';
				}
