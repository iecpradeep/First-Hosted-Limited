 ...
 * Script Type:	Library
 *
 * Version:	1.0.0 - 21/02/2013 - created - JM
 * 			1.0.1 - 25/07/2012 - convertToFloat function added - AS
 * 			1.0.2 - AR rule added - JM
 *
 * Author:	FHL
...
{	
	...
	try
	{
		...
		if(custIntID>0)
		{
			...
			defBillLine = customerRecord.findLineItemValue('addressbook', 'defaultbilling', 'T');
			billCountry = customerRecord.getLineItemText('addressbook', 'country', defBillLine);

	
			
			vatOverride = customerRecord.getFieldValue('custentity_vatnumberoverride');
			vatValid = customerRecord.getFieldValue('custentity_vatnumbervalid');
...
function jsDate_To_nsDate_Reverse(jsdate)// used for dateConv to reverse of the NS Date (format : yyyy/mm/dd)
{	  	
	var theDay = jsdate.getDate();
	var theMonth = jsdate.getMonth()+1;
	var theYear = jsdate.getFullYear();

	var nsdate_reverse = theYear+"/"+theMonth+"/"+ theDay;

	return nsdate_reverse;
...
/**
 * reverse date i.e. 2012-05-17 to 17.05.2012
...
/**
 * unreverse date i.e. 17.05.2012 to 2012-05-17
 */
function UNreverseDate(dateStr)
{
	var dateArr = null;
	var newDate='';
	...
	if (dateArr[0].length == 1) //Day
	{
		dateArr[0] = '0' + dateArr[0];
	}
	
	if (dateArr[1].length == 1) //Month
	{
 ...
 * 
 * modified to handle the inter company tag also
 * 
 */
function getCustAcctsReceivableID(billAddID, channel)
{
	var returnARaccountID = null;
	var customerARaccount = '';
	...
	try 
	{
		if (billAddID) 
		{
			...
			if (channel != channel.replace(/.*distributor.*/gmi, "")) 
			{
	...
	{
		errorHandler('getCustAcctsReceivableID', e);
	}
