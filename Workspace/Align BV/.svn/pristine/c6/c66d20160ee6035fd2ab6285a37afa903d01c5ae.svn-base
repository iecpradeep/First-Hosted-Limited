...
var ORDERLEVELITEMID = 1384; // Fixed placeholder ID
...
function discountsEngine()
{
	...
	try
	{
		...
		// Only engage discounts process if order is not FOC
		if (parseFloat(estimateTotalBefore) > 0.00)
		{
...
function findAnyDiscountForScheme()
{
	try
	{
		...
		// applied
		var schemeID = genericSearch('customrecord_alignbv_pricediscountscheme', 'name', schemeRow.getValue('name'));
		var schemeTypeID = schemeRow.getValue('custrecord_alignbv_scheme_basedisc_type');
		var schemeLevel = getSchemeLevel(schemeTypeID);
		var schemeRate = getSchemeRate(schemeTypeID);
		var schemeCountries = schemeRow.getValue('custrecord_alignbv_scheme_countrylisttxt');
		// T/F whether to use coupons custom table
		var schemeUseCoupons = schemeRow.getValue('custrecord_alignbv_scheme_coupon_lookup'); 
		...
		{
			schemeRowDateArray[d] = nlapiLookupField('customrecord_alignbv_discount_schemedate', schemeDateSelectArray[d],'custrecord_alignbv_scheme_datefield_id');
			schemeDatesDebugText += schemeRowDateArray[d] + " ";
		}

		schemeRowDateFrom = schemeRow.getValue('custrecord_alignbv_scheme_effectivefrom');
		schemeRowDateTo = schemeRow.getValue('custrecord_alignbv_scheme_effectiveto');
		...
		// defined in sequences of saved search(es) to test

		var searchDiscscountTierFilters = new Array();
		searchDiscscountTierFilters[0] = new nlobjSearchFilter('custrecord_alignbv_tier_type', null, 'anyof', (schemeID));

		searchDiscountTierRows = nlapiSearchRecord('customrecord_alignbv_tiers_discounts', 'customsearch_alignbv_discount_tiers', searchDiscscountTierFilters, null);

		if (searchDiscountTierRows)
		{ 
			// Try each eligible tier in turn - set the tier parameters first			
			for (var thisTier = 0; thisTier < searchDiscountTierRows.length && !isEligible; thisTier++)
			{
				var tierRow = searchDiscountTierRows[thisTier];
				var tierSavedSearchID = tierRow.getValue('custrecord_alignbv_tier_criteria_search');
				var tierSavedSearchName = tierRow.getText('custrecord_alignbv_tier_criteria_search');
				var tierSeqNo = tierRow.getValue('custrecord_alignbv_tier_level_sequence');
				var tierMin = tierRow.getValue('custrecord_alignbv_tier_min_count');
				...
				var tierMax = tierRow.getValue('custrecord_alignbv_tier_max_count');
				...
				var tierFrequency = tierRow.getValue('custrecord_alignbv_tier_frequency');
				...
				{
					var searchifEligibleObj = nlapiLoadSearch('', tierSavedSearchID);
				...
				switch (schemeLevel)
				{
					...
					case SCHEMECUSTOMERTYPE:
						...
						//var entityChildCriteria = new Array('entity', 'anyof', customerInternalID);
						...
						if (parentEntityID)
						{
							...
							//var entityParentCriteria = new Array('entity', 'anyof', parentCustomerInternalID);
					...
					if (schemeRowDateFrom && !schemeRowDateTo)
					{
						for ( var d = 0; d < schemeRowDateArray.length; d++)
						{
							if (d > 0)
							{
								...
								var dateRangeMultiCriteria = new Array(schemeRowDateArray[d], 'onorafter', schemeRowDateFrom);
					...
					else
					{
						if (!schemeRowDateFrom && schemeRowDateTo)
						{
							for ( var d = 0; d < schemeRowDateArray.length; d++)
							{
								...
								{
									dateRangeCriteria.push('and');
									var dateRangeMultiCriteria = new Array(schemeRowDateArray[d], 'onorbefore', schemeRowDateTo);
									dateRangeCriteria.push(dateRangeMultiCriteria);
								}
							}
						} 
						else // Both date ranges present - use within
						{
							for ( var d = 0; d < schemeRowDateArray.length; d++)
							{
								...
								{
									dateRangeCriteria.push('and');
									var dateRangeMultiCriteria = new Array(schemeRowDateArray[d], 'within', schemeRowDateFrom, schemeRowDateTo);
									dateRangeCriteria.push(dateRangeMultiCriteria);
								}
							}
						}
				...
				{
					criteria.push('and');
					var countryCriteria = new Array('custbody_alignbv_billing_region', 'anyof', schemeRowCountriesArray);
				...
				if (isEligible)
				{
					discountItemID = schemeRow.getValue('custrecord_alignbv_scheme_base_item');
					discountItemRecord = nlapiLoadRecord('discountitem', discountItemID);
					discountItemRate = tierRow.getValue('custrecord_alignbv_tier_base_discount');

					if (schemeRate == SCHEMEPERCENTRATE)
					{
						discountItemRate += "%";
					}
					...
					itemObj.name = discountItemRecord.getFieldValue('itemid');
...
{	
	...
	try
	{
		currentLineItem = thisEstimateItem + itemLevelDiscountCounter;
		var thisEstimateItemid = estimateRecord.getLineItemValue('item', 'item', currentLineItem);
		var thisEstimateItemAmount = estimateRecord.getLineItemValue('item', 'amount', currentLineItem);
		...
		for (var thisDiscountItem = 1; thisDiscountItem <= discountItemsArray.length; thisDiscountItem++)
		{
			...
			if (debugOn)
			{
				nlapiLogExecution('DEBUG', "applyAnyItemLevelDiscount(" + thisEstimateItem + ")" + thisEstimateItem + "/" + itemListCount, "currentLineItem = " + currentLineItem + " thisEstimateItemid = " + thisEstimateItemid  + " DiscItemCode(s) = " + thisDiscountItemObj.itemcode);
			}
			...
			{	
				if (itemCodes[thisDiscountItemCode] == thisEstimateItemid)
				{
					...
					{
						estimateRecord.insertLineItem('item', currentLineItem + 1);
					}

					estimateRecord.setCurrentLineItemValue('item', 'itemtype', 'Discount');
					estimateRecord.setCurrentLineItemValue('item', 'description', thisDiscountItemObj.desc);
					estimateRecord.setCurrentLineItemValue('item', 'item', thisDiscountItemObj.internalid);
					estimateRecord.setCurrentLineItemValue('item', 'rate', discountAmountRate);
					estimateRecord.commitLineItem('item');

					thisDiscountItemObj.discountAmount = estimateRecord.getLineItemValue('item', 'amount', currentLineItem);

					if (debugOn)
					{
						nlapiLogExecution('DEBUG', "Line Index " + estimateRecord.getCurrentLineItemIndex('item'), "currentLineItem = " + currentLineItem + " amount = " + thisDiscountItemObj.discountAmount);
					}

					// Assemble the item answer elements
					XMLItemAnswer = createTaggedXML(XMLORDITEMINDEX, thisEstimateItem);
					XMLItemAnswer += createTaggedXML(XMLDISCITEMAMOUNT,	thisDiscountItemObj.discountAmount);
					XMLItemAnswer += createTaggedXML(XMLDISCITEMID,	thisDiscountItemObj.internalid);
					XMLItemAnswer += createTaggedXML(XMLDISCITEMRATE, discountAmountRate);
					XMLItemAnswer += createTaggedXML(XMLDISCITEMDESC, thisDiscountItemObj.name);
					
					// Apply wrapper XML
					XMLItemAnswer = createTaggedXML(XMLITEMDISCOUNT, XMLItemAnswer);
					// Append to existing XML Item Answer list
					XMLItemAnswerList += XMLItemAnswer;

					if (debugOn) 
					{
						nlapiLogExecution('DEBUG', "XML Answer " + thisDiscountItem, XMLItemAnswer);
						nlapiLogExecution('DEBUG', "Discount Item Level " + thisDiscountItem + "/" + discountItemsArray.length + " : Item ("
...
function applyAnyOrderLevelDiscount()
{
	...
	try
	{
		...
		for ( var thisDiscountItem = 1; thisDiscountItem <= discountItemsArray.length; thisDiscountItem++)
		{
			...
			if (thisDiscountItemObj.schemelevel == SCHEMEORDERTYPE || thisDiscountItemObj.schemelevel == SCHEMECUSTOMERTYPE)
			{
				discountAmountRate = thisDiscountItemObj.rate;
				
				if (!isNaN(discountAmountRate))
				{
					discountAmountRate = parseFloat(discountAmountRate).toFixed(2);
				}
				
				estimateRecord.setFieldValue('discountitem', thisDiscountItemObj.internalid);

				orderLevelDiscountTotal = parseFloat(orderLevelDiscountTotal) + parseFloat(discountAmountRate);
				...
				estimateRecord.selectNewLineItem('item');
				estimateRecord.setCurrentLineItemValue('item', 'itemtype', 'Description');
				estimateRecord.setCurrentLineItemValue('item', 'item', ORDERLEVELITEMID);
				estimateRecord.setCurrentLineItemValue('item', 'description', thisDiscountItemObj.name + " : " + discountAmountRate);
				estimateRecord.commitLineItem('item');
		...
		if (orderLevelDiscountCounter > 0)
		{
			...
			estimateRecord.setFieldValue('discountrate', orderLevelDiscountTotal);
...
function processEstimatetoSalesOrder()
{
	try
	{
		...
		if (estimateObj)
		{
			...
			estimateObj.setFieldValue('memo', "Discount calc : Before : " + estimateTotalBefore + " After : " + estimateTotalAfter);
			estimateID = nlapiSubmitRecord(estimateObj, false);

			if (estimateID > 0 && transformToSalesOrder)
			{
				salesOrderObj = nlapiTransformRecord('estimate', estimateID, 'salesorder', {recordmode : 'dynamic'});
				...
				// Update the coupons record if applicable
				if (salesOrderID && couponsEligibilityID)
				{
					...
					var couponData = new Array('Align Order# ' + AlignOrderNumber + " - " + AlignOrderDate,	salesOrderID, todayDate);
					nlapiSubmitField('customrecord_alignbv_discount_coupons', couponsEligibilityID,	couponFields, couponData, false);
					
					if (debugOn)
					{
						nlapiLogExecution('DEBUG', "Coupon Updated: " + couponsEligibilityID, 'salesOrderID: ' + salesOrderID + ' patientID: ' + patientID);					
					}
				}
				
				nlapiSubmitField('customrecord_tibco_order_feed', payloadRecordID, 'custrecord_alignbv_ordernumber', salesOrderID, false);
				
				nlapiSubmitField('customrecord_tibco_order_feed', payloadRecordID, 'custrecord_alignbv_salesorder_lookup', salesOrderID, false);				
				...
				if (salesOrderID > 0 && deleteAfterTransform)
				{
					...
					if (debugOn)
					{
						nlapiLogExecution('DEBUG', "delete Estimate", 'internalid: ' + estimateID);
					}
				}
			}
		}
	...
	try
	{
		...
		switch (recordType)
		{
			case 'salesorder':
				...
				payloadRecordID = genericSearch('customrecord_tibco_order_feed', 'custrecord_alignbv_salesorder_lookup', estimateID);
				estimateRecord = nlapiLoadRecord('salesorder', estimateID);
				break;
				
			case 'customrecord_fhlevents':
				stagingRecordID = nlapiGetRecordId();
				estimateID = nlapiLookupField(recordType, stagingRecordID, 'custrecord_fhle_salesorder');
				payloadRecordID = genericSearch('customrecord_tibco_order_feed', 'custrecord_alignbv_salesorder_lookup', estimateID);
			...
			case 'customrecord_tibco_order_feed':
				...
				estimateID = nlapiLookupField('customrecord_tibco_order_feed', payloadRecordID, 'custrecord_tibco_order_estimate_lookup');
				estimateRecord = nlapiLoadRecord('estimate', estimateID);
				break;
		...
		isManualOrder = nlapiLookupField('customrecord_tibco_order_feed', payloadRecordID, 'custrecord_tibco_order_is_manualentry');
		...
		//parentEntityID = estimateRecord.getFieldText('parent');
		//parentCustomerInternalID = estimateRecord.getFieldValue('parent');
		parentEntityID = nlapiLookupField('customer', customerInternalID, 'parent', true);
		parentCustomerInternalID = nlapiLookupField('customer', customerInternalID, 'parent', false);
		customerChannel = nlapiLookupField('customer', customerInternalID, 'custentity_region', true);
		customerARaccount = null;
		
		patientID = estimateRecord.getFieldValue('custbody_patientnumber');
		if (patientID != null) // Extract any ID between (...) e.g. (12235)
...
function getCustomerAcctsReceivableID()
{
	...
	try 
	{
		if (billingAddressCountryID) 
		{
			...
			if (customerChannel != customerChannel.replace(/.*distributor.*/gmi, "")) 
			{
			...
			returnARaccountID = genericSearch('account', 'number', customerARaccount);
		}
	}
	catch (e)
	{
...
function setDeploymentParameters()
{
	try
	{
		...
		{
			deleteAfterTransform = true; 
		}
		else
		{
			deleteAfterTransform = false;
		}
	}
	catch (e)
	{
...
function completeAndSaveXMLDiscountAnswer()
{
	...
	nlapiSubmitField('customrecord_tibco_order_feed', payloadRecordID, 'custrecord_alignbv_discounts_xmlcalc', XMLDiscountAnswer, false);
...
function logCriteria(criteria)
{
	...
	nlapiLogExecution('DEBUG', criteria.length + " criteria", criteriaText);
}
...
function addToErrorsXML(errorSource, errorDesc)
{
	...
	try
	{
		...
		nlapiSubmitField('customrecord_tibco_order_feed', payloadRecordID, 'custrecord_alignbv_discounts_xmlerrors', createTaggedXML(XMLERRORLIST, XMLErrors), false);
		//status = 'Failed';
