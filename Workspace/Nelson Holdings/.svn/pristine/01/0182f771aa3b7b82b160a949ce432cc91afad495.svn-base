 ...
 * 1.0.4 - new block "updatedDespositedamount"
 * 
 ******************/
function updatedDepositedAmount(salesOrderID)
{
	...
	try
	{
		...
		CDSearchFilters = new Array();
		CDSearchColumns = new Array();

		CDSearchFilters[0] = new nlobjSearchFilter('salesorder', null, 'anyof', salesorder);

		CDSearchColumns[0] = new nlobjSearchColumn('fxamount');   //1.0.1 
		CDSearchColumns[1] = new nlobjSearchColumn('gcoconfirmedrefundedtotal'); 
		...
		if (CDSearchResults)
		{
			...
			{			
				...
				totalRefunded += parseFloat(CDSearchResults[j].getValue(CDSearchColumns[1]));
