 ...
 *              1.1.4 - 24/04/2013 - Added new block "updatedDespositedamount" -SA
 *
 * Author:		FHL
...
function genericSearch(table, fieldToSearch, valueToSearch)
{
	...
	{
		invoiceSearchColumns[0] = new nlobjSearchColumn('internalid');
		invoiceSearchColumns[1] = new nlobjSearchColumn('deposittransaction');

		// perform search
		var itemSearchResults = nlapiSearchRecord('depositapplication', null, invoiceSearchFilters, invoiceSearchColumns);
...
function updatedDepositedAmount(salesOrderID)
{
	...
	try
	{
		...
		nlapiSubmitField('salesorder', salesOrderID, 'custbody7', netDepositAmount.toFixed(2), false); 
		nlapiSubmitField('salesorder', salesOrderID, 'custbody_netpercentdepositted', percentDepositted.toFixed(2), false); 
