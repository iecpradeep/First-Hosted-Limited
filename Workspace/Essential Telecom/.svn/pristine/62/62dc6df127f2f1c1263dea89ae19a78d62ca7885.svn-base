 ...
 *					1.1.2 - 25/02/2013 - splitting the voucher numbers - AS
 *					1.1.3 - 14/03/2013 - set the distriCommission to two decimal point - AS
 *				
 * Author: 			FHL
 * Purpose: 		To print the voucher details in invoice separately.
 * 		
 * Script: 			custscript_printvouchers
 * Deploy: 			customdeploy_printvouchers
 * 
 * Library: 		library.js
 * 
 **************************************************************************************************************************************************************************/
...
function printVouchers()
{
	...
	try
	{
		...
		{
			var errormsg = "<html><body><p>You cannot print the Vouchers as they have not been saved yet, or the Parameter has been passed through incorrectly.</p></body></html>";
		...
		{	
			...
			pdfxml += '<pdf>';
			pdfxml += '<head><meta name="title" value="invoice Vouchers" />\n<style type="text/css">.style1{height: 23px;}</style></head>';
			...
			pdfxml += '<p style="font-family: Arial, Helvetica, sans-serif; font-size: large; font-weight: bold;">invoice Vouchers</p>';
			...
			pdfxml += '<td style="font-family: Arial, Helvetica, sans-serif; font-size: x-small; background-color: Silver;"><b>Voucher Numbers</b></td>'; 
			pdfxml += '</tr>';

			//Looping through each item line
			for(var i = 1; i <= numOfItems; i++)
			{
				// getting the value in item list's 'Voucher' column to check whether there are vouchers
				vouchers = invoiceRecord.getLineItemValue('item', 'custcol_vouchers', i);		
				
				//if vouchers are there 
				if(vouchers != null)
				{
					...
					//spliting the voucher numbers (voucher numbers are returned as one line value, hence needs to be splitted)
					forEachVoucherRecord();
					
					pdfxml = pdfxml.concat('</td></tr>');	
					//pdfxml +='<td style="font-family: Arial, Helvetica, sans-serif; font-size: x-small">' + itemVouchers + '</td></tr>';
			...
			try
			{
				...
				response.setContentType('PDF','invoiceVouchers.pdf');
 ...
 *
 *	version 1.1.2 - splitting the voucher numbers - AS
 */
...
function forEachVoucherRecord()
{
	...
	try
	{
		...
		voucherArray = itemVouchers.split('\n');
		
		for(var i = 0; i< voucherArray.length; i++)
		{
			//seetting the nnumbers separately in the table line with a break tab
			setVoucherNumber(voucherArray[i]);
			
		}
	}
	catch(e)
	{
		errorHandler("forEachVoucherRecord", e);		
	}

}


/*setting each voucher number in a new line
 * 
 * version 1.1.2 - splitting the voucher numbers - AS
 * @param voucherNumber - each voucher Number in each line
 */
...
function getInvoiceLineFields(lineNumber)
{
	...
	try
	{
		...
		itemVouchers = nlapiEscapeXML(invoiceRecord.getLineItemValue('item', 'custcol_vouchers', lineNumber));
