 ...
 *         		3.5.0  - 11/06/2013 - Added germany to the system, testing to ensue - AN
 *         	    3.5.1  - 02/07/2013 - Minor tweaks to the system - AN
 *         		3.5.2  - 05/07/2013 - Fixing the calculation issue introduced in 3.4.18 - AN
 *         
 *
...
// german search variables - 3.5.0
var gerSearchFilters = new Array();
var gerSearchColumns = new Array();
var germanySearchResults = "";
			...
			switch(parseInt(expenseCategory))
			{
				...
				case 10: // Subsistence
					...
					else
					{
						...
						// 3.5.2 - no longer used and was causing an error
						//record.setFieldValue('custrecord_exp_exchangerate', exchangeRate);
...
function subsistenceStepOne(assistant,iteration,lineEdit)
{
	try
	{
		...
		var subsistenceTravelUs = assistant.addField("subsistenceradiobutton", "radio", "US", '3',"groupsubsistencetravel").setLayoutType('midrow');
		var subsistenceTravelGermany = assistant.addField("subsistenceradiobutton", "radio", "Germany", '4',"groupsubsistencetravel").setLayoutType('endrow');
	...
	if (projectSearchResults)
	{
		...
		switch (parseInt(subsistenceUkTravel))
		{
			case 1: // ROW
				...
				assistant.addFieldGroup("groupsubsistencecountry", "Where are you travelling?");
			...
			case 4: // Germany
				...
				// this should be skipped for UK and germany
				break;
...
function subsistenceStepFive(assistant,iteration,lineEdit, baseCurrencyText,employeeLocationId)
{
	...
	switch(parseInt(subsistenceUkTravel))
	{
		...
		case 3: // US
			...
			nlapiLogExecution('DEBUG', 'step 5 USA employeeLocationId', employeeLocationId);
		...
		case 4: // Germany 3.5.0
			
			nlapiLogExecution('DEBUG', 'Germany step 5');
			
			// get and store the per diem rates for this employee
			getEmployeePerDiemRate(employeeLocationId);
			gerSearchFilters[0] = new nlobjSearchFilter('internalid', null, 'is', employeeLocationId);
			
			nlapiLogExecution('DEBUG', 'Got employee per diem rates');
	
			// do the search
			germanySearchResults = nlapiSearchRecord('customrecord_perdiemrates', null, gerSearchFilters, gerSearchColumns);

			if(germanySearchResults == null)
			{
				// no search results
				nlapiLogExecution('DEBUG', 'Found no perdiem rates for germany', null);
			}
			else
			{        
				// get rates
				var eightHourRate = germanySearchResults[0].getValue(gerSearchColumns[0]);
				var fourteenHourRate = germanySearchResults[0].getValue(gerSearchColumns[1]);
				var twentyFourHourRate = germanySearchResults[0].getValue(gerSearchColumns[2]);

				//check rates script side
				nlapiLogExecution('DEBUG','Germany eight rate', eightHourRate);   
				nlapiLogExecution('DEBUG','Germany fourteen rate ', fourteenHourRate);    
				nlapiLogExecution('DEBUG','Germany twentyfour rate ', twentyFourHourRate);  
			}

			// work out percentages for deductions                
			var twentyPercent = ((20/100) * twentyFourHourRate);
			nlapiLogExecution('DEBUG', 'Germany twentyPercent', twentyPercent);  
			var twentyPercentround = Round(twentyPercent);
			nlapiLogExecution('DEBUG', 'Germany twentyPercentround', twentyPercentround);
			// 3.5.1 - AN edit
			var twentyPercentFixed = toFixed(twentyPercent,2);

			var fourtyPercent = ((40/100) * twentyFourHourRate);
			nlapiLogExecution('DEBUG', 'Germany fourtyfourPercent', fourtyPercent);  
			var fourtyPercentRound = Round(fourtyPercent);
			nlapiLogExecution('DEBUG', 'Germany twentyeightpercent rounded', twentyeightPercentRound);  
			var fourtyPercentFixed = toFixed(fourtyPercent,2);
			...
			// populate the rates table 
			var oneMealRateLabel = assistant.addField("onemealratelabel", "label", "Eight hour rate: " + eightHourRate, null,"grouprates");
			var twoMealRateLabel = assistant.addField("twomealratelabel", "label", "Fourteen hour rate: " + fourteenHourRate, null,"grouprates");
			var maxRateLabel = assistant.addField("maxratelabel", "label", "Max rate: " + twentyFourHourRate, null,"grouprates");

			// single day calculation 
			if(subsistenceDateDiff < 0)
			{
				nlapiLogExecution('DEBUG','Germany subsistenceDepartureAMPM', subsistenceDepartureAMPM);
				nlapiLogExecution('DEBUG','Germany subsistenceReturnAMPM', subsistenceReturnAMPM);

				var strSplitTime = parseInt(subsistenceDepartureTime.replace(":",""));
				nlapiLogExecution('DEBUG','Germany time in hundreds', strSplitTime);	
				...
				var strSplitReturnTime = parseInt(subsistenceReturnTime.replace(":",""));          	
				nlapiLogExecution('DEBUG','Germany timehundred day 2', strSplitReturnTime);
				...
				else
				{
					if(totalTime < 1000)
					{
						var subsistenceFiveHourRate = assistant.addField("subsistencefiverate", "label", "Eight hour: " + eightHourRate, null, "grouppartialdailyallowanceoone");
						var partialDayOneTotal = parseInt(eightHourRate);
						var subsistencePartialOneTotal = assistant.addField("subsistencepartialdayonetotal", "label", "Partial first day total: " + partialDayOneTotal, null, "grouppartialdailyallowanceoone");	
					}
					else
					{
						if(totalTime < 2400)
						{
							var subsistenceTenHourRate = assistant.addField("subsistencetenrate", "label", "Fourteen hour: " + fourteenHourRate, null, "grouppartialdailyallowanceoone");
							var partialDayOneTotal = parseInt(fourteenHourRate);
							var subsistencePartialOneTotal = assistant.addField("subsistencepartialdayonetotal", "label", "Partial first day total: " + partialDayOneTotal, null, "grouppartialdailyallowanceoone");
						}
						else
						{
							if(totalTime >= 2400)
							{
								var subsistenceTwentyFourHour = assistant.addField("subsistencetwentyrate", "label", "Max rate: " + twentyFourHourRate, null, "grouppartialdailyallowanceoone");
								var partialDayOneTotal = parseInt(twentyFourHourRate);
								var subsistencePartialOneTotal = assistant.addField("subsistencepartialdayonetotal", "label", "Partial first day total: " + partialDayOneTotal, null, "grouppartialdailyallowanceoone");    	       
							}
							else
			...
			else // multiple day calculation
			{
				...
				var fulldaytotal = subsistenceDateDiff * twentyFourHourRate;
				nlapiLogExecution('DEBUG','Germany fulldaytotal', fulldaytotal);
				...
				nlapiLogExecution('DEBUG','Germany timehundred', strSplitTime);				
				nlapiLogExecution('DEBUG','Germany subsistenceDepartureTime', subsistenceDepartureTime);
				...
				if(subsistenceDepartureAMPM == 'AM') //zzz
				{
					nlapiLogExecution('DEBUG','Germany AM', null);
					// if starts at 12am (24:00) this becomes a 24 hour journey
					if(strSplitTime == 1200)
					{
						var subsistenceTwentyFourHour = assistant.addField("subsistencetwentyrate", "label", "Max rate: " + twentyFourHourRate, null, "grouppartialdailyallowanceoone");
					...
					else // 3.5.2 - the journey started between 12:30 and 11:30 AM 
					{						// 3.5.2 - time equal to 1230 or greater than 1am? 
						if(strSplitTime == 1230 || strSplitTime >= 100)
						{
							// Version number: 3.5.2 - AN 
							var subsistenceTenHourRate = assistant.addField("subsistencetenrate", "label", "Fourteen hour: " + fourteenHourRate, null, "grouppartialdailyallowanceoone");
							var partialDayOneTotal = parseInt(fourteenHourRate);
							var subsistencePartialOneTotal = assistant.addField("subsistencepartialdayonetotal", "label", "Partial first day total: " + partialDayOneTotal, null, "grouppartialdailyallowanceoone");
						}
				...
				else
				{
					if(subsistenceDepartureAMPM == 'PM')
					{
						nlapiLogExecution('DEBUG','Germany PM', null);
						// if trip starts before 2PM
						if(strSplitTime <= 200 || strSplitTime >= 1200)
						{
							var subsistenceTenHourRate = assistant.addField("subsistencetenrate", "label", "fourteen hour: " + fourteenHourRate, null, "grouppartialdailyallowanceoone");
				...
				if(subsistenceReturnAMPM == 'AM')
				{
					...
					else
					{
						...
						if(strSplitReturnTime < 1000)
						{
							var subsistenceFiveRateTwo = assistant.addField("susbsistencefiveratetwo", "label", "Eight hour: " + eightHourRate, null,"grouppartialdailyallowancetwo");    		        		
							var partialDayTwoTotal = parseInt(eightHourRate);
							var subsistencePartialTwoTotal = assistant.addField("subsistencepartialdaytwototal", "label", "Partial last day total: " + partialDayTwoTotal, null, "grouppartialdailyallowancetwo");
						}						
						else
						{
							if(strSplitReturnTime >= 1000) // if greater than 10 am give 5,10 and breakfast rates
							{
								var subsistenceTenRateTwo = assistant.addField("susbsistencetenratetwo", "label", "fourteen hour: " + fourteenHourRate, null,"grouppartialdailyallowancetwo");
								var partialDayTwoTotal = parseInt(fourteenHourRate);					
								var subsistencePartialTwoTotal = assistant.addField("subsistencepartialdaytwototal", "label", "Partial last day total: " + partialDayTwoTotal, null, "grouppartialdailyallowancetwo");
							}
						}
					}
			...
			if(subsistenceMealsTrue == 1)
			{
				...
				var subsistenceDeductBreakfastLabel = assistant.addField("subsistencedeductbreakfast", "label", "Breakfast: " + twentyPercentFixed + " x " + subsistenceBMeal, null, "groupdeductions");
				deductionsTotal += twentyPercentFixed*subsistenceBMeal;
				nlapiLogExecution('DEBUG', 'subsistence deductionsTotal', deductionsTotal);

				var subsistenceDeductOneMealRate = assistant.addField("subsistencedeductonemealrate", "label", "Lunch: " + fourtyPercentFixed  + " x " + subsistenceLMeal, null, "groupdeductions");
				deductionsTotal += fourtyPercentFixed*subsistenceLMeal;
				nlapiLogExecution('DEBUG', 'subsistence deductionsTotal', deductionsTotal);

				var subsistenceDeductTwoMealRate = assistant.addField("subsistencededucttwomealrate", "label", "Dinner: " + fourtyPercentFixed  + " x " + subsistenceDMeal, null, "groupdeductions");
				deductionsTotal += fourtyPercentFixed*subsistenceDMeal;
				
				deductionsTotal = toFixed(deductionsTotal,2);

				nlapiLogExecution('DEBUG', 'subsistence deductionsTotal', deductionsTotal);

				// 3.5.1 - removed for german per diemcalc
				...
				var subsistenceHiddenOneMealAmount= assistant.addField("subsistenceamountonemeal", 'integer', 'hidden').setDisplayType('hidden').setDefaultValue(parseInt(fourteenPercentround)*parseInt(subsistenceLMeal)); 
				var subsistenceHiddenTwoMealAmount= assistant.addField("subsistenceamounttwomeal", 'integer', 'hidden').setDisplayType('hidden').setDefaultValue(parseInt(twentyeightPercentRound)*parseInt(subsistenceDMeal)); 
				var subsistenceHiddenLateMealAmount= assistant.addField("subsistenceamountlatemeal", 'integer', 'hidden').setDisplayType('hidden').setDefaultValue(0); 
			}
...
/**
 * Usage: take the employe location and search versus the table
 * @param employeeLocationId - an employee location ID stored in customrecord_employeelocation
 */
function getEmployeePerDiemRate(employeeLocationId)
{
	nlapiLogExecution('DEBUG', 'get german per diems');
	...
	switch(parseInt(employeeLocationId))
	{
		case 1: // Uk staff	
			...
			gerSearchColumns[0] = new nlobjSearchColumn('custrecord_ukfiverate');
			gerSearchColumns[1] = new nlobjSearchColumn('custrecord_uktenrate'); 
			gerSearchColumns[2] = new nlobjSearchColumn('custrecord_stg'); 
		...
		case 2: // US staff
			...
			gerSearchColumns[0] = new nlobjSearchColumn('custrecord_usfiverate');
			gerSearchColumns[1] = new nlobjSearchColumn('custrecord_ustenrate');                
			gerSearchColumns[2] = new nlobjSearchColumn('custrecord_usd'); 
		...
		case 3: // can staff
			...
			gerSearchColumns[0] = new nlobjSearchColumn('custrecord_canfiverate');
			gerSearchColumns[1] = new nlobjSearchColumn('custrecord_cantenrate');                
			gerSearchColumns[2] = new nlobjSearchColumn('custrecord_cad'); 
		...
		case 4: // europe staff
			...
			gerSearchColumns[0] = new nlobjSearchColumn('custrecord_eufiverate');
			gerSearchColumns[1] = new nlobjSearchColumn('custrecord_eutenrate');                
			gerSearchColumns[2] = new nlobjSearchColumn('custrecord_eur'); 
		...
		case 5: // sri lanka staff
			...
			gerSearchColumns[0] = new nlobjSearchColumn('custrecord_slfiverate');
			gerSearchColumns[1] = new nlobjSearchColumn('custrecord_sltenrate');                
			gerSearchColumns[2] = new nlobjSearchColumn('custrecord_usdsrilanka'); 
		...
		case 6: // SINGAPORE
			...
			gerSearchColumns[0] = new nlobjSearchColumn('custrecord_sgpfiverate');
			gerSearchColumns[1] = new nlobjSearchColumn('custrecord_sgptenrate');                
			gerSearchColumns[2] = new nlobjSearchColumn('custrecord_sgp'); 

			break; // case 6 end

		case 7: // GERMANY

			nlapiLogExecution('DEBUG', 'Germany & employeeLocationId', employeeLocationId);

			gerSearchColumns[0] = new nlobjSearchColumn('custrecord_germanyeightrate');
			gerSearchColumns[1] = new nlobjSearchColumn('custrecord_germanyfourteenrate');                
			gerSearchColumns[2] = new nlobjSearchColumn('custrecord_germany'); 
		...
		case 8: // SPARE TWO
			...
			gerSearchColumns[0] = new nlobjSearchColumn('custrecord_sparetwofiverate');
			gerSearchColumns[1] = new nlobjSearchColumn('custrecord_sparetwotenrate');                
			gerSearchColumns[2] = new nlobjSearchColumn('custrecord_sparetwo'); 
		...
		case 9: // SPARE THREE
			...
			gerSearchColumns[0] = new nlobjSearchColumn('custrecord_sparethreefiverate');
			gerSearchColumns[1] = new nlobjSearchColumn('custrecord_sparethreetenrate');                
			gerSearchColumns[2] = new nlobjSearchColumn('custrecord_sparethree'); 
		...
		case 10: // SPARE FOUR
			...
			gerSearchColumns[0] = new nlobjSearchColumn('custrecord_sparefourfiverate');
			gerSearchColumns[1] = new nlobjSearchColumn('custrecord_sparefourtenrate');                
			gerSearchColumns[2] = new nlobjSearchColumn('custrecord_sparefour'); 
		...
		case 11: // SPARE FIVE
			...
			gerSearchColumns[0] = new nlobjSearchColumn('custrecord_sparefivefiverate');
			gerSearchColumns[1] = new nlobjSearchColumn('custrecord_sparefivetenrate');                
			gerSearchColumns[2] = new nlobjSearchColumn('custrecord_sparefive'); 
		...
		case 12: // SPARE SIX
			...
			gerSearchColumns[0] = new nlobjSearchColumn('custrecord_sparesixfiverate');
			gerSearchColumns[1] = new nlobjSearchColumn('custrecord_sparesixtenrate');                
			gerSearchColumns[2] = new nlobjSearchColumn('custrecord_sparesix'); 
		...
		case 13: // SPARE SEVEN
			...
			gerSearchColumns[0] = new nlobjSearchColumn('custrecord_sparesevenfiverate');
			gerSearchColumns[1] = new nlobjSearchColumn('custrecord_spareseventenrate');                
			gerSearchColumns[2] = new nlobjSearchColumn('custrecord_spareseven'); 
		...
		case 14: // SPARE EIGHT
			...
			gerSearchColumns[0] = new nlobjSearchColumn('custrecord_spareeightfiverate');
			gerSearchColumns[1] = new nlobjSearchColumn('custrecord_spareeighttenrate');                
			gerSearchColumns[2] = new nlobjSearchColumn('custrecord_spareeight'); 
		...
		case 15: // SPARE NINE
			...
			gerSearchColumns[0] = new nlobjSearchColumn('custrecord_spareninefiverate');
			gerSearchColumns[1] = new nlobjSearchColumn('custrecord_spareninetenrate');                
			gerSearchColumns[2] = new nlobjSearchColumn('custrecord_sparenine'); 
		...
		case 16: // SPARE TEN
			...
			gerSearchColumns[0] = new nlobjSearchColumn('custrecord_sparetenfiverate');
			gerSearchColumns[1] = new nlobjSearchColumn('custrecord_sparetentenrate');                
			gerSearchColumns[2] = new nlobjSearchColumn('custrecord_spareten'); 
		...
		case 17: // SPARE ELEVEN
			...
			gerSearchColumns[0] = new nlobjSearchColumn('custrecord_spareelevenfiverate');
			gerSearchColumns[1] = new nlobjSearchColumn('custrecord_spareeleventenrate');                
			gerSearchColumns[2] = new nlobjSearchColumn('custrecord_spareeleven'); 
		...
		case 18: // SPARE TWELVE
			...
			gerSearchColumns[0] = new nlobjSearchColumn('custrecord_sparetwelvefiverate');
			gerSearchColumns[1] = new nlobjSearchColumn('custrecord_sparetwelvetenrate');                
			gerSearchColumns[2] = new nlobjSearchColumn('custrecord_sparetwelve'); 
		...
		case 19: // SPARE thirteen
			...
			gerSearchColumns[0] = new nlobjSearchColumn('custrecord_sparethirteenfiverate');
			gerSearchColumns[1] = new nlobjSearchColumn('custrecord_sparethirteentenrate');                
			gerSearchColumns[2] = new nlobjSearchColumn('custrecord_sparethirteen'); 
		...
		case 20: // SPARE fourteen
			...
			gerSearchColumns[0] = new nlobjSearchColumn('custrecord_sparefourteenfiverate');
			gerSearchColumns[1] = new nlobjSearchColumn('custrecord_sparefourteentenrate');                
			gerSearchColumns[2] = new nlobjSearchColumn('custrecord_sparefourteen'); 
